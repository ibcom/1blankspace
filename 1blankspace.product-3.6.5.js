...
ns1blankspace.product = 
{
	...
	refresh: function (oParam, oResponse)
	{
		if (oResponse == undefined)
		{
			...
			oSearch.method = 'PRODUCT_PRICE_SEARCH';
			oSearch.addField('effectivefromdate,effectivetodate,price,costprice,pricegroup,pricegrouptext,pricetype,pricetypetext');
			oSearch.addFilter('product', 'EQUAL_TO', ns1blankspace.objectContext);
			oSearch.rows = 100;
			oSearch.sort('effectivefromdate', 'desc');
			oSearch.getResults(function(data)
			{
				ns1blankspace.product.refresh(oParam, data)
			});
		}
		else
		{
			ns1blankspace.product.data.pricing = oResponse.data.rows;

			if (ns1blankspace.objectContextData != undefined)
			{
				ns1blankspace.objectContextData.currentretailprice = '';
				ns1blankspace.objectContextData.currentretailcostprice = '';
				ns1blankspace.objectContextData.currentwholesaleprice = '' ;
				ns1blankspace.objectContextData.currentwholesalecostprice = '' ;

				var oProducePrice = _.find(oResponse.data.rows, function (row) {return row.pricetype == 1});
				if (oProducePrice != undefined) {ns1blankspace.objectContextData.currentretailprice = oProducePrice.price}

				oProducePrice = _.find(oResponse.data.rows, function (row) {return row.pricetype == 2});
				if (oProducePrice != undefined) {ns1blankspace.objectContextData.currentwholesaleprice = oProducePrice.price}

				oProducePrice = _.find(oResponse.data.rows, function (row) {return row.pricetype == 1});
				if (oProducePrice != undefined) {ns1blankspace.objectContextData.currentretailcostprice = oProducePrice.costprice}

				oProducePrice = _.find(oResponse.data.rows, function (row) {return row.pricetype == 2});
				if (oProducePrice != undefined) {ns1blankspace.objectContextData.currentwholesalecostprice = oProducePrice.costprice}
			}
	...
	layout: 	function ()
	{
		...
		else
		{
			...
			if (ns1blankspace.product.option.manageStock)
			{
				...
				aHTML.push('<tr><td id="ns1blankspaceControlStockHistory" class="ns1blankspaceControl">' +
								'<div>Adjustments</div><div class="ns1blankspaceSubNote">history</div></td></tr>');
		...
		$('#ns1blankspaceControlPricing').click(function(event)
		{
			...
			ns1blankspace.product.pricing.init();
		});
	...
	show:	function (oParam, oResponse)
	{
		...
		else
		{
			...
			oParam.onComplete = ns1blankspace.product.showContext;
			ns1blankspace.product.refresh(oParam)
		}
	},
			
	showContext: function (oParam, oResponse)
	{			
		var aHTML = [];
		...
		{
			aHTML.push('<div id="ns1blankspaceControlContext_reference" class="ns1blankspaceSub">' + ns1blankspace.objectContextData.reference + '</div>');
		}
		
		if (ns1blankspace.objectContextData.categorytext != '')
		{
			aHTML.push('<div id="ns1blankspaceControlContext_category" class="ns1blankspaceSub">' + ns1blankspace.objectContextData.categorytext + '</div>');
		}
		
		if (ns1blankspace.objectContextData.currentretailprice != '')
		{
			aHTML.push('<div id="ns1blankspaceControlContext_price" class="ns1blankspaceSub" style="margin-top:8px;">$' + ns1blankspace.objectContextData.currentretailprice + '</div>' +
								'<div class="ns1blankspaceSubNote" style="font-size:0.5em;">RETAIL</div>');
		}

		if (ns1blankspace.objectContextData.currentwholesaleprice != '')
		{
			aHTML.push('<div id="ns1blankspaceControlContext_wholesaleprice" class="ns1blankspaceSub" style="margin-top:8px;">$' + ns1blankspace.objectContextData.currentwholesaleprice + '</div>' +
								'<div class="ns1blankspaceSubNote" style="font-size:0.5em;">WHOLESALE</div>');
		}
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			if (ns1blankspace.objectContextData.currentretailprice != '')
			{
				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Retail Sell Price</td></tr>' +
			...
			if (ns1blankspace.objectContextData.currentwholesaleprice != '')
			{
				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Wholesale Sell Price</td></tr>' +
							'<tr><td id="ns1blankspaceSummaryPrice" class="ns1blankspaceSummary">$' +
							ns1blankspace.objectContextData.currentwholesaleprice +
							'</td></tr>');
			}

			if (ns1blankspace.objectContextData.currentretailcostprice != '')
			{
				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Retail Cost Price</td></tr>' +
							'<tr><td id="ns1blankspaceSummaryPrice" class="ns1blankspaceSummary">$' +
							ns1blankspace.objectContextData.currentretailcostprice +
							'</td></tr>');
			}

			if (ns1blankspace.objectContextData.currentwholesalecostprice != '')
			{
				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Wholesale Cost Price</td></tr>' +
							'<tr><td id="ns1blankspaceSummaryPrice" class="ns1blankspaceSummary">$' +
							ns1blankspace.objectContextData.currentwholesalecostprice +
							'</td></tr>');
			}
	...
	details:	function ()
	{
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<textarea id="ns1blankspaceDetailsDescription" style="width:100%;" rows="5" cols="35" class="ns1blankspaceTextMulti"></textarea>' +
							'</td></tr>');
			...
			aHTML.push('<tr class="ns1blankspaceCaption" style="display:none;">' +
							'<td class="ns1blankspaceCaption">' +
							'Type' +
							'</td></tr>' +
							'<tr class="ns1blankspace" style="display:none;">' +
							'<td class="ns1blankspaceRadio">' +
							'<input type="radio" id="radioProductType1" name="radioProductType" value="1"/>Standard' +
							'<br /><input type="radio" id="radioProductType2" name="radioProductType" value="2"/>Group' +
							'</td></tr>');
			...
			if (ns1blankspace.objectContextData != undefined)
			{
				...
				$('#ns1blankspaceDetailsFinancialAccountProductSales')
					.val(ns1blankspace.objectContextData.financialaccountincometext.formatXHTML())
					.attr("data-id", ns1blankspace.objectContextData.financialaccountincome);
				$('#ns1blankspaceDetailsFinancialAccountProductPurchases')
					.val(ns1blankspace.objectContextData.financialaccountpurchasestext.formatXHTML())
					.attr("data-id", ns1blankspace.objectContextData.financialaccountpurchases);
			}
			else
			{
	...
	{			
		init: function ()
		{
			if ($('#ns1blankspaceMainPricing').attr('data-loading') == '1')
			{
				...
				aHTML.push('<table class="ns1blankspaceContainer">' +
								'<tr class="ns1blankspaceContainer">' +
								'<td id="ns1blankspacePricingColumn1">' + ns1blankspace.xhtml.loading + '</td>' +
								'<td id="ns1blankspacePricingColumn2" style="width:300px;"></td>' +
								'</tr>' +
								'</table>');
		...
		show: function(oParam, oResponse)
		{
			...
			if (oResponse == undefined)
			{
				var oSearch = new AdvancedSearch();
				oSearch.method = 'PRODUCT_PRICE_SEARCH';
				oSearch.addField('effectivefromdate,effectivetodate,price,costprice,pricegroup,pricegrouptext,pricetype,pricetypetext,notes');
				oSearch.addFilter('product', 'EQUAL_TO', ns1blankspace.objectContext);
				oSearch.rows = 100;
				oSearch.sort('effectivefromdate', 'desc');
				oSearch.getResults(function(data)
				{
					ns1blankspace.product.pricing.show(oParam, data)
				});
			}
			else
			{
				ns1blankspace.objectContextData.priceHistory = oResponse.data.rows;

				if (oResponse.status == 'OK')
				{
					if (oResponse.data.rows.length == 0)
					{
						aHTML.push('<div class="ns1blankspaceSub">No pricing</div>');
					}
					else
					{
						aHTML.push('<table id="ns1blankspaceProductPricing" class="ns1blankspace" style="font-size:1em;">');
						aHTML.push('<tr>' +
										'<td class="ns1blankspaceHeaderCaption">Type</td>' + 
										'<td class="ns1blankspaceHeaderCaption">Sell Price</td>' + 
										'<td class="ns1blankspaceHeaderCaption">Cost Price</td>' +
										'<td class="ns1blankspaceHeaderCaption">Effective From</td>' + 
										'<td class="ns1blankspaceHeaderCaption">Effective To</td>' +
										'<td class="ns1blankspaceHeaderCaption" style="text-align:right;">&nbsp;</td>' + 
									'</tr>');

						//'<td class="ns1blankspaceHeaderCaption">Group</td>' +

						$.each(oResponse.data.rows, function(r, oRow)
						{
							aHTML.push('<tr>' + 
											'<td class="ns1blankspaceRow">' + this.pricetypetext + '</td>' +
											'<td class="ns1blankspaceRow">' + this.price + '</td>' +
											'<td class="ns1blankspaceRow">' + this.costprice + '</td>' +
											'<td class="ns1blankspaceRow">' + '<div>' + ns1blankspace.util.fd(this.effectivefromdate, {format: 'DD MMM YYYY'}) + '</div>' +
																							'<div class="ns1blankspaceSubNote">' + ns1blankspace.util.fd(this.effectivefromdate, {format: 'h:mm A'}) + '</div></td>' +
											'<td class="ns1blankspaceRow">' + '<div>' + ns1blankspace.util.fd(this.effectivetodate, {format: 'DD MMM YYYY'}) + '</div>' +
																							'<div class="ns1blankspaceSubNote">' + ns1blankspace.util.fd(this.effectivetodate, {format: 'h:mm A'}) + '</div></td>' +
											'<td style="width:70px;text-align:right;" class="ns1blankspaceRow">' + 
												'<span id="ns1blankspaceProductPricing_remove-' + oRow.id + '" class="ns1blankspaceProductPricingRemove" style="margin-right:2px;"></span>' +
												'<span id="ns1blankspaceProductPricing_edit-' + oRow.id + '" class="ns1blankspaceProductPricingEdit"' +
													' data-effectivefromdate="' + oRow["effectivefromdate"] + '"' +
													' data-effectivetodate="' + oRow["effectivetodate"] + '"' +
													' data-price="' + oRow["price"] + '"' +
													' data-costprice="' + oRow["costprice"] + '"' +
													' data-pricegroup="' + oRow["pricegroup"] + '"' +
													' data-pricegrouptext="' + oRow["pricegrouptext"] + '"' +
													' data-pricetype="' + oRow["pricetype"] + '"' +
													' data-pricetypetext="' + oRow["pricetypetext"] + '"' +
													' data-notes="' + window.btoa(oRow["notes"]) + '"' +
													'></span></td>' +
											'</tr>');
						});

						//	'<td class="ns1blankspaceRow">' + this.pricegrouptext + '</td>' +
					...
					aHTML.push('<table class="ns1blankspaceColumn2">' +
										...
										'<span id="ns1blankspaceProductPricingAdd" class="ns1blankspaceAction">Add</span>' +
										'</td></tr>' +
										'</table>');

					$('#ns1blankspacePricingColumn2').html(aHTML.join(''));

					$('#ns1blankspaceProductPricingAdd').button(
					{
						label: "Add"
					})
					.click(function()
					{
						ns1blankspace.product.pricing.edit({xhtmlElementID: this.id})
					})
					.css('width', '60px');
				}

				ns1blankspace.product.pricing.bind(oParam);
			}
		},
		...
		bind: function ()
		{
			...
			$('#ns1blankspaceProductPricing .ns1blankspaceProductPricingRemove').button(
			{
				text: false,
				icons:
				{
			...
			.click(function()
			{
				ns1blankspace.remove(
				{
					...
					method: 'PRODUCT_PRICE_MANAGE',
					ifNoneMessage: 'There is no product pricing.'
				});
			})
			.css('width', '15px')
			.css('height', '17px');

			$('#ns1blankspaceProductPricing .ns1blankspaceProductPricingEdit:visible').button(
			{
				text: false,
				icons:
				{
		...
		edit:	function (oParam, oResponse)
		{
			...
			aHTML.push('<tr class="ns1blankspaceContainer">' +
							'<td id="ns1blankspaceProductPricingEditColumn1"></td>' +
							'<td id="ns1blankspaceProductPricingEditColumn2" style="width:50px;"></td>' +
							'</tr>');
			...
			aHTML.push('<table class="ns1blankspaceColumn2a">' +
							'<tr><td><span id="ns1blankspaceProductPricingEditSave" data-id="' + iID + '" class="ns1blankspaceAction">' +
							'Save</span></td></tr>' +
							'<tr><td><span id="ns1blankspaceProductPricingEditCancel" class="ns1blankspaceAction">' +
							'Cancel</span></td></tr>' +
							'</table>');					
			
			$('#ns1blankspaceProductPricingEditColumn2').html(aHTML.join(''));
			
			$('#ns1blankspaceProductPricingEditSave').button(
			{
				text: "Save"
			})
			.click(function()
			{
				ns1blankspace.product.pricing.save({id: $(this).attr('data-id')})
			})
			.css('width', '65px');

			$('#ns1blankspaceProductPricingEditCancel').button(
			{
				text: "Cancel"
			})
			.click(function()
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<tr class="ns1blankspace">' +
							'<td class="ns1blankspaceText">' +
							'<input id="ns1blankspaceProductPricingEditPrice" class="ns1blankspaceText">' +
							'</td></tr>');

			aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption">' +
							'<div>Cost Price</div>' +
							'<div class="ns1blankspaceSubNote">optional</div>' +
							'</td></tr>' +
							'<tr class="ns1blankspace">' +
							'<td class="ns1blankspaceText">' +
							'<input id="ns1blankspaceProductPricingEditCostPrice" class="ns1blankspaceText">' +
							'</td></tr>');
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<tr class="ns1blankspace">' +
							'<td class="ns1blankspaceRadio">' +
							'<input type="radio" id="radioType1" name="radioType" value="1"/>Retail' +
							'<br /><input type="radio" id="radioType2" name="radioType" value="2"/>Wholesale' +
							'</td></tr>');

			aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption">' +
							'<div>Effective From</div>' +
							'<div class="ns1blankspaceSubNote">leave blank for today</div>' +
							'</td></tr>' +
							'<tr class="ns1blankspace">' +
							'<td class="ns1blankspaceDate">' +
							'<input id="ns1blankspaceProductPricingEditEffectiveFromDate" class="ns1blankspaceDate">' +
							'</td></tr>');
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<tr class="ns1blankspace">' +
							'<td class="ns1blankspaceDate">' +
							'<input id="ns1blankspaceProductPricingEditEffectiveToDate" class="ns1blankspaceDate">' +
							'</td></tr>');
			...
			// 				'<tr class="ns1blankspace">' +
			// 				'<td class="ns1blankspaceText">' +
			// 				'<input id="ns1blankspaceProductPricingEditPriceGroup" class="ns1blankspaceSelect"' +
			// 					' data-method="SETUP_CONTACT_PRICE_GROUP_SEARCH"' +
			// 					' data-columns="title">' +
			// 				'</td></tr>');
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<td class="ns1blankspaceTextMulti">' +
							'<textarea rows="10" cols="35" id="ns1blankspaceProductPricingEditNotes" class="ns1blankspaceTextMulti"></textarea>' +
							'</td></tr>');

			aHTML.push('</table>');		

			$('#ns1blankspaceProductPricingEditColumn1').html(aHTML.join(''));

			$('#ns1blankspaceProductPricingEditPrice').focus();
						
			ns1blankspace.util.initDatePicker({select: 'input.ns1blankspaceDate', time: true});

			if (iID != '')
			{
				$('#ns1blankspaceProductPricingEditEffectiveFromDate').val(ns1blankspace.util.getData(oParam, 'data-effectivefromdate').value);
				$('#ns1blankspaceProductPricingEditEffectiveToDate').val(ns1blankspace.util.getData(oParam, 'data-effectivetodate').value);
				$('#ns1blankspaceProductPricingEditPrice').val(ns1blankspace.util.getData(oParam, 'data-price').value);
				$('#ns1blankspaceProductPricingEditCostPrice').val(ns1blankspace.util.getData(oParam, 'data-costprice').value);
				//$('#ns1blankspaceProductPricingEditPriceGroup').val(ns1blankspace.util.getData(oParam, 'data-pricegrouptext').value);
				//$('#ns1blankspaceProductPricingEditPriceGroup').attr('data-id', ns1blankspace.util.getData(oParam, 'data-pricegroup').value);
				$('[name="radioType"][value="' + ns1blankspace.util.getData(oParam, 'data-pricetype').value + '"]').attr('checked', true);
				$('#ns1blankspaceProductPricingEditNotes').val(window.atob(ns1blankspace.util.getData(oParam, 'data-notes').value));	 
			}
			else
			{
		...
		save: function (oParam, oResponse)
		{
			if (oResponse == undefined)
			{
				...
				{
					id: iID,
					effectivefromdate: $('#ns1blankspaceProductPricingEditEffectiveFromDate').val(),
					price: $('#ns1blankspaceProductPricingEditPrice').val(),
					costprice: $('#ns1blankspaceProductPricingEditCostPrice').val(),
					pricetype: $('[name="radioType"]:checked').val(),
					notes: $('#ns1blankspaceProductPricingEditNotes').val()
				}

				//pricegroup: $('#ns1blankspaceProductPricingEditPriceGroup').attr('data-id'),

				if ($('#ns1blankspaceProductPricingEditEffectiveToDate').val() != '')
				{
					oData.effectivetodate = $('#ns1blankspaceProductPricingEditEffectiveToDate').val();
				}


				if ($('#ns1blankspaceProductPricingEditEffectiveFromDate').val() == '')
				{
					oData.effectivefromdate = moment().format('DD MMM YYYY HH:mm A');
				}

				if (iID == '')
				{
					oData.product = ns1blankspace.objectContext;
				}
				...
				$.ajax(
				{
					...
					success: function(data) 
					{
						if (data.status == 'OK')
						{
							...
							ns1blankspace.product.pricing.show();
							ns1blankspace.product.refresh({onComplete: ns1blankspace.product.showContext});
						}
					}
				});
			}
		...
		group: function (oParam, oResponse)
		{
			...
			{
				var oSearch = new AdvancedSearch();
				oSearch.method = 'SETUP_CONTACT_PRICE_GROUP_SEARCH';
				oSearch.addField('title');
				oSearch.sort('title', 'ASC');
				oSearch.rows = 99999;
				oSearch.getResults(function(data)
				{
					ns1blankspace.product.pricing.group(oParam, data)
				});
			}
			else
			{
	...
	stock:		
	{
		show: 		function ()
		{
			...
			if ($('#ns1blankspaceMainStock').attr('data-loading') == '1')
			{
				...
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								'<td class="ns1blankspaceCaption">' +
								'Stock Purchase ' + ns1blankspace.option.taxVATCaption + ' Type' +
								'</td></tr>' +
								'<tr class="ns1blankspace">' +
								'<td class="ns1blankspaceText" id="ns1blankspaceStockTaxType">' +
								'</td></tr>');	
				...
				var iTaxType = (ns1blankspace.objectContextData && ns1blankspace.objectContextData.supplierordertaxtype) 
								? ns1blankspace.objectContextData.supplierordertaxtype : '1';
				ns1blankspace.financial.util.tax.codes(
				{
					xhtmlElementID: 'ns1blankspaceStockTaxType',
					id: iTaxType
				});
				var aHTML = [];
				...
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								...
								'</td></tr>' +
								'<tr class="ns1blankspace">' +
								'<td class="ns1blankspaceRadio" id="ns1blankspaceStockUnitType">' +
				...
				ns1blankspace.product.stock.unitTypes();
				...
				if (ns1blankspace.objectContextData != undefined)
				{
					...
					$('[name="radioTrackStock"][value="' + (ns1blankspace.objectContextData.trackinventory || (ns1blankspace.product.option.manageStock?'Y':'N')) + '"]').attr('checked', true);
					var cUnitPrice= ns1blankspace.objectContextData.unitprice;
					if (cUnitPrice == '') {cUnitPrice = 'Unknown'}
					$('#ns1blankspaceStockUnitCostPrice').html(cUnitPrice);
		...
		unitTypes: function()
		{
			...
			var oSearch = new AdvancedSearch();
			oSearch.method = 'SETUP_PRODUCT_UNIT_TYPE_SEARCH';
			oSearch.addField('title');
			oSearch.rows = 100;
			oSearch.getResults(function(oResponse)
			{
				if (oResponse.status == 'OK')
				{
					if (oResponse.data.rows.length > 10)
					{
						aHTML.push('<input id="ns1blankspaceStockUnitTypeSelect" class="ns1blankspaceSelect"' +
										'data-method="SETUP_PRODUCT_UNIT_TYPE_SEARCH" data-methodFilter="title-TEXT_IS_LIKE">');
						sHTML = aHTML.join('');
					}
					else
					{
						$.each(oResponse.data.rows, function()
						{
							aHTML.push('<input type="radio" id="radioStockUnit' + this.id + '"' +
											' name="radioStockUnit" value="' + this.id + '"/>' 
				...
				$('#ns1blankspaceStockUnitType').html(sHTML);

				if (ns1blankspace.objectContextData != undefined)
				{
					if ($('[name="radioStockUnit"]').length != 0)
					{	
						$('[name="radioStockUnit"][value="' + ns1blankspace.objectContextData.unittype + '"]').attr('checked', true);
					}
					else
					{ 
						$('#ns1blankspaceProductUnitTypeSelect')
						.val(ns1blankspace.objectContextData.unittypetext)
						.attr('data-id', ns1blankspace.objectContextData.unittype)
					}
				}
				else
				{
					...
					{
						$('#ns1blankspaceStockUnitTypeSelect').val('Each').attr('data-id', '1')
					}
				}
			});
		},
			...
			row:	function (oRow, oParam)
					{
						...
						aHTML.push('<td id="ns1blankspaceStockHistory_effectivedate-' + oRow.id + '" class="ns1blankspaceRow">' +
											ns1blankspace.util.fd(oRow.effectivedate) + '</td>');
	...
	{			
		send: function ()
		{
			...
			if ($('#ns1blankspaceMainStock').html() != '')
			{
				...
				oData.supplierordertaxtype = $('#ns1blankspaceMainStock input[name="radioTaxCode"]:checked').val()
			}
			...
			{
				oData.customerordertaxtype = $('#ns1blankspaceMainPricing input[name="radioTaxCode"]:checked').val();;
			}
		...
		priceHistoryCheck: function (oParam)
		{
			var aPriceHistory = _.filter(ns1blankspace.objectContextData.priceHistory, function(oPriceHistory)
			{	
				return (oPriceHistory.effectivetodate == '')
			});

			if (aPriceHistory.length > 0)
			{
				var oData =
				{
					id: _.first(aPriceHistory).id,
					effectivetodate: moment().add('d', 0).format('DD MMM YYYY HH:mm:ss')
				}
				...
				$.ajax(
				{
					...
					success: function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							ns1blankspace.product.save.price(oParam)
						}
						else
						{
							ns1blankspace.status.error('There is an issue with the pricing history: ' + oResponse.error.errornotes);
						}
					}
				});
			}
		...
		price: function (aParam)
		{
			...
			if ($('#ns1blankspacePricingPriceRetail').val() != ns1blankspace.objectContextData.currentretailprice)
			{
				oData.price = $('#ns1blankspacePricingPriceRetail').val();
				oData.product = ns1blankspace.objectContext;
				oData.group = '-1';
				oData.effectivefromdate = moment().add('s', 1).format('DD MMM YYYY HH:mm:ss')
				...
				$.ajax(
				{
					...
					success: function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							$('#ns1blankspaceMainPricing').attr('data-loading', '1')
							delete(ns1blankspace.objectContextData.priceHistory);
							ns1blankspace.objectContextData.currentretailprice = oData.price;
							ns1blankspace.product.pricing.show();
						}
						else
						{
							ns1blankspace.status.error('Unable to save price: ' + oResponse.error.errornotes);
						}
					}
				});
			}
