 ...
 * Licensed as Attributiondefaultaccount-ShareAlike 4.0 International
 * http://creativecommons.org/licenses/by-sa/4.0/
 */

//toDo - add processing for account and systemwide document templates
// ToDo: When viewing Deleted Folder, indicate that only last 25 messages are shown and that user can get more by clicking on 'Get More' button
...
ns1blankspace.messaging.imap = 
{
	...
	{
		// v2.0.611 Added so that delete in Control area would work
	...
	folders:
	{
		data:
		{
			names:
			{
				deleted: ['Trash', 'Deleted', 'Bin'],	/* v2.0.614 Added 'Bin' */
		...
		folder:
		{
			...
			{		
				...
				{	
					...
					// v2.0.610 now sends URL to enable threading during CHECK call
	...
	{		
		...
		{	
			if (!ns1blankspace.messaging.checking)
			{
				...
				{
					ns1blankspace.status.working('Checking for new emails...', {timeout: false});
				}	
		...
		if (ns1blankspace.messaging.imap.emailAccounts.length == 0)
		{
			...
			else
			{
				...
				if (oResponse.data.rows.length != 0)
				{
					...
					$.each(oResponse.data.rows, function(index)
					{
						...
						{	
							...
							{
								// v2.0.610 SUP022741 Set default account to person's email address account 
								...
								//else if (index == ns1blankspace.messaging.imap.emailAccounts.length - 1)
							}
							else
							{
				...
				$('td.ns1blankspaceControl').click(function(event)
				{
					...
					else if (sID == 'Saved')		// v2.0.6h SUP022648 Added Saved actions
					{	
						ns1blankspace.show({selector: '#ns1blankspaceMainActionsSaved'});
						ns1blankspace.messaging.imap.actions.show({xhtmlElementID: 'ns1blankspaceMainActionsSaved', type: 9, sent: 'N', xhtmlContext: 'ActionsSaved'});
					}
				...
				{	
					...
					if (ns1blankspace.messaging.imap.data.defaultAccount != undefined && bAutoShow)
					{
						// v2.0.611 SUP023127 Need to make sure it does exactly the same as on click event of email - wasn't getting folders first.
						ns1blankspace.messaging.imap.account = undefined;
						$('#ns1blankspaceMessaging-' + ns1blankspace.messaging.imap.account).addClass('ns1blankspaceHighlight');
						$('#ns1blankspaceMessaging-' + ns1blankspace.messaging.imap.data.defaultAccount).click();
					}
					else
					{
	...
	{				
		checkForEditing: function(oParam)
		{
			...
			if (sText != '')
			{
				// v2.0.613 Need to use dialog.destroy otherwise widget classes are still associated
		...
		show:		function (oParam, oResponse)
		{
			...
			else
			{
				...
				// v2.0.612 SUP023198 Now passes deleted parameter
		...
		{	
			...
			// v2.0.6g SUP022568 now saves row's details to inbox row so we an retrieve these if search is not complete
			aHTML.push('<tr id="ns1blankspaceMessagingInbox_id-' + sID + '" class="ns1blankspaceRow' + sClass + '" data-cached="' + oRow.detailscached + '"' + 
										' data-row="' + JSON.stringify(oRow).replace(/"/g, "~~") + 
										'">');
			...
			aHTML.push('<td id="ns1blankspaceMessagingInbox_subject-' + sID + 
								'" style="padding-right:5px;" class="ns1blankspaceRow ns1blankspaceMainRowSelect' + sClass + '">' +
								oRow.subject + '</td>');
			...
			show:		function (oParam)
			{
				...
				if (iMessageID)
				{
					aHTML.push('<tr><td class="ns1blankspace">' +
									'<input type="checkbox" id="ns1blankspaceMessageSaveContacts" /> Create contacts' +
									'</td></tr>');
				...
				aHTML.push('<tr><td style="padding-left:25px;" class="ns1blankspaceCaption">' +
								'Subject</td></tr>');
			...
			search: 	function (oParam, oResponse)
			{
				...
				else
				{
					...
					.click(function()
					{
						...
						$('#ns1blankspaceMessageSaveObjectContext').val($(this).html().formatXHTML());		// v2.0.610 Was showing &amp;
	...
	search: 	
	{
		send:		function (sXHTMLElementID, oParam)
		{
			...
			var iRows = 10;
			var iAccount = (ns1blankspace.messaging.imap.account!=undefined
							? ns1blankspace.messaging.imap.account
							:ns1blankspace.messaging.imap.data.defaultAccount);
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				// v2.0.6g SUP022568 save data that we currently know to objectContextData - this is saved against the inbox row
				if ($('#ns1blankspaceMessagingInbox_id-' + ns1blankspace.objectContext).is('*'))
				{
					ns1blankspace.objectContextData = $('#ns1blankspaceMessagingInbox_id-' + ns1blankspace.objectContext).attr('data-row');
					if (ns1blankspace.objectContextData) {ns1blankspace.objectContextData = JSON.parse(ns1blankspace.objectContextData.replace(/~~/g, '"'))};
				}
				ns1blankspace.messaging.action = -1;
			...
			else
			{
				...
				if (sSearchText.length >= iMinimumLength || iSource == ns1blankspace.data.searchSource.browse)
				{
					...
					// v2.0.6c added from and fromname filter
					// v2.0.6f Added message filter
					var oSearch = new AdvancedSearch();
					oSearch.method = 'MESSAGING_EMAIL_CACHE_SEARCH';
	...
	show: 		function (oParam, oResponse)
	{
		...
		else
		{
			...
			ns1blankspace.objectContext = ns1blankspace.objectContextData.id;		// v2.0.6g wasn't being reset and causing wrong email to be used. Also update UI row with new data
			if ($('ns1blankspaceMessagingInbox_id-' + ns1blankspace.objectContext).is('*'))
			{
				$('ns1blankspaceMessagingInbox_id-' + ns1blankspace.objectContext).attr('data-row', JSON.stringify(ns1blankspace.objectContextData).replace(/"/g, '~~'));
			}
	...
	summary:	function ()
	{
		...
		else
		{
			...
			if (ns1blankspace.objectContextData.to != '')
			{
				...
				// v2.0.6h SUP022760 Was erroring if any were blank
				$.each(aTo, function(i)
				{	
					if (this != '')
					{
			...
			if (ns1blankspace.objectContextData.detailscached == 'Y')
			{
				...
				// v2.0.6g If message had been cached but not viewed, it would never be set to Seen (doesn't apply to ACTIONS though)
			...
			{						
				$.ajax(
				{
					...
					success: function(oResponse) 
					{
						// v2.0.6h SUP022521 We need to make sure that the user hasn't moved to a different email while this call was in progress
						if (ns1blankspace.objectContext == oResponse.id)		//Can't do this until Chad adds the id to the response...
						{
							ns1blankspace.objectContextData.detailscached = 'Y';
							ns1blankspace.objectContextData.message = oResponse.message;
							ns1blankspace.objectContextData.attachments = oResponse.attachmentlist;		// v2.0.6 changed from attachmentslist
							ns1blankspace.objectContextData.hasattachments = (parseInt(oResponse.attachmentcount) > 0) ? 'Y' : 'N';	//v2.0.6 was returning boolean
							ns1blankspace.objectContextData.sourcetypetext = 'EMAIL';
	...
	message: 	
	{
		...
		edit: 		
		{
				...
				if (oParam.continueEditing != true)
				{
					if (bNewEmail)
					{
						...
						else
						{
							...
							// 2.0.6g If user has clicked too quickly, objectContextData will have the wrong values. Get what we know and default the rest
							/*if (ns1blankspace.objectContext != ns1blankspace.objectContextData.id)
							{
								// Get information for objectContextData from row in inbox
								ns1blankspace.objectContextData.to = '|' + ns1blankspace.messaging.imap.emailAccount.email;
								ns1blankspace.objectContextData.subject = $('#ns1blankspaceMessagingInbox_subject-' + ns1blankspace.objectContext).html();
								ns1blankspace.objectContextData.fromname = $('#ns1blankspaceMessagingInbox_from-' + ns1blankspace.objectContext).html();
								ns1blankspace.objectContextData.from = $('#ns1blankspaceMessagingInbox_from-' + ns1blankspace.objectContext).attr('title');
								ns1blankspace.objectContextData.date = $('#ns1blankspaceMessagingInbox_date-' + ns1blankspace.objectContext).html().split('<br').shift() + ' ' + 
																		$('#ns1blankspaceMessagingInbox_date-' + ns1blankspace.objectContext).children().last().html();;
								ns1blankspace.objectContextData.detailscached = $('#ns1blankspaceMessagingInbox_from-' + ns1blankspace.objectContext).attr('data-cached');
								ns1blankspace.objectContextData.subject = $('#ns1blankspaceMessagingInbox_subject-' + ns1blankspace.objectContext).html();
								ns1blankspace.objectContextData.hasattachments = '?';
								ns1blankspace.objectContextData.attachments = "undefined";
								ns1blankspace.objectContextData.imapflags = '()';
								ns1blankspace.objectContextData.messageid = undefined;
								ns1blankspace.objectContextData.id = ns1blankspace.objectContext;
							}*/
						}
						...
						// v2.0.6g Now uses objectContext instead of objectContextData.id as this is set in search.send
						// v2.0.6h Reverted back to objectContextData.id
						if (ns1blankspace.objectContextData === undefined
						...
						else
						{
							$.ajax(
							{
								...
								success: function(oResponse)
								{
									if (oResponse.status === 'OK')
									{
										// ??? ToDo Do we need to set objectContext and change values in objectContextData here?
										ns1blankspace.messaging.action = oResponse.action;
										// v2.0.6f Sets objectContextData.id to id of action for correctness
										//ns1blankspace.objectContextData.id = oResponse.action;
					...
					.click(function()
					{
						...
						else 
						{
							...
							{	// v2.0.6h SUP022410 Added sEmail as IE didn't like $(this).html()
					...
					if (ns1blankspace.objectContextData != undefined && iSource == 1)
					{
						...
						if (ns1blankspace.objectContextData != undefined)
						{
							...
							{
								// v2.0.614 Now respondes to replytoemail
			...
			{
				// v2.0.610 SUP022741 Find footer matching current email account, or if footer is coming from currently logged on person, 
				...
				if (aSearchIDs.length > 0)
				{
					...
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status === 'OK')
						{
							...
							{
								// v2.0.6h Had to make email lower case and remove leading/trailing spaces for accurate comparison
								$.each(aRecipientsTo, function()
								{
									var sEmail = this.toString().toLowerCase().replace(/ /g, '');
									var oThisContact = $.grep(aContacts, function(x) {return x.email.toLowerCase().replace(/ /g, '') == sEmail}).shift();
			...
			contents: 	function (oParam)
			{
				...
				else
				{
					...
					if (oParam.draft != true)
					{
						...
						aHTML.push('<br /><br />---- Original Message ----<br />');
						...
						// v2.0.6h Now gets addresses in one command and splits at #, not |
						var sOrgTo = $.map(ns1blankspace.objectContextData.to.split('#'), function(x) {return x.split('|').pop()}).join('; ');
						
						aHTML.push(sOrgTo + '</td></tr>');
						
						var sOrgCc = $.map(ns1blankspace.objectContextData.cc.split('#'), function(x) {return x.split('|').pop()}).join('; ');
						
						if (sOrgCc != '')
						{
							aHTML.push('<tr><td><strong>CC:</strong> ' + sOrgCc + '</td></tr>');	
						}
			...
			attach: 	
			{
				show:		function (oParam, oResponse)
				{
					...
					{	
						// v2.0.6 Only need to call this if we don't already have an action id (changed from MESSAGING_EMAIL_DRAFT)
		...
		multipleRecipients: 
		{
			choose: function()
			{
				...
				for (var i = 1; i <= 3; i++)
				{
					...
					var aTextAreaEmails = $(sElementID).val().replace(/ /g, "").split(';');		// v2.0.6h SUP022658 remove spaces first

					// Now populate aDisplayEmails with contactperson id and email of each email in textarea. Make sure case is the same
					// v2.0.609 SUP022894 Need to reduce email from To element to lowerCase for comparison purposes
				...
				$('.ns1blankspaceMessagingSaveRecipient').on('click', function()
				{
					if ($(this).prop('checked'))
					{
						if ($('#ns1blankspaceMessageSendSaveObjectContext').val() == '')
						{
							...
							$('#ns1blankspaceMessageSendSaveObjectContext').val($(this).attr('data-value').formatXHTML());	// v2.0.610 SUP023067
						}
					}
					else
					{
		...
		send:		function (oParam)
		{
			...
			else
			{
						...
						else
						{
							...
							.click(function()
							{
								...
								if ($('#ns1blankspaceMessageSendSaveObject').prop('checked'))
								{
									...
									// v2.0.6 also saves against business / person if we have this info
				...
				{	
					...
					if (oParam.object == '32' && oParam.contactPersonTo) {oData.saveagainstobjectcontext = oParam.contactPersonTo.split('-').shift();}	//v2.0.6d
					if (oParam.objectContext) 
					{
						oData.saveagainstobjectcontext = oParam.objectContext.split('-').shift();
					}

					// v2.0.6d SUP022302 Was giving error if one supplied without the other
					if (oData.saveagainstobject && oData.saveagainstobjectcontext == undefined)
					{
						delete(oData.saveagainstobject);
					}
					// v2.0.6g Was only sending save parameter if oParam save was true
					// v2.0.6h Only send this parameter when not a draft
					if (oParam.draft != true)
					{
		...
		object: 	function (oParam, oResponse)
		{
			...
			else
			{
				...
				.click(function()
				{
					...
					$('#ns1blankspaceMessageSendSaveObjectContext').val($(this).html().formatXHTML());	// v2.0.610 SUP023067
					$('#ns1blankspaceMessageSendSaveObjectContextSearch').html('');
					$('#ns1blankspaceMessageSendSaveObject').prop('checked', true);
				});
		...
		{	
			...
			{	
				...
				// v2.0.611 SUP023131 Need to determine if we're deleting a saved email or a CACHE email and then act accordingly
				...
				// v2.0.6 Also added error handling
				$.ajax(
				{
					type: 'POST',
					url: ns1blankspace.util.endpointURI(sMethod),
	...
	actions: 	
	{
		show: 		function (oParam, oResponse)
		{
			...
			if (oResponse == undefined)
			{
				...
				oSearch.addFilter('emailsent', 'EQUAL_TO', sSent);
			...
			else
			{
				...
				if (oResponse.data.rows.length == 0)
				{
					aHTML.push('<table><tr><td class="ns1blankspaceNothing">No ' + ((sSent === 'Y') ? 'sent' : 'draft') + ' emails.</td></tr></table>');
				}
				else
				{
					aHTML.push('<table>');
