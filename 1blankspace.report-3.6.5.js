...
ns1blankspace.report = 
{
	...
	show:		
	{
		main: function (oParam, oResponse)
		{
			...
			oParam.searchAtTop = ((Object.keys(oFixedParameters).length == 0 || !oParam.showFixedParameters
								|| (oFixedParameters.fields && oFixedParameters.fields.length > 0)) 
			...
			if (ns1blankspace.report.config.autoRun == true)
			{
				...
				if (oParam.scriptReportSearch)
				{
					oParam.scriptReportSearch(oParam);
				}
				else
				{
			...
			else
			{
				...
				else if (oResponse != undefined)
				{
					...
					ns1blankspace.report.show.headerTabs();
					aHTML.push(ns1blankspace.report.show.configurationHeader({searchAtTop: oParam.searchAtTop}));
					...
					{
						if ($('#ns1blankspaceReportCheckAll').prop('checked')) 
						{
			...
			else
			{
				...
				if (oSearchParameters && oSearchParameters.filters)
				{
					$.each(oSearchParameters.filters, function(i, t)
					{
						var bInclude = true;
						var oFilter = {};
						// 3.6.5 Removed references to applytosubsearch
						oFilter.name = t.name;
