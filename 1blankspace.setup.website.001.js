...
function interfaceSetupWebsiteViewport()
{
	...
	else
	{
		...
		aHTML[++h] = '<tr id="trInterfaceViewportControl" class="interfaceViewportControl">' +
						'<td id="tdInterfaceViewportControlAddAttachment" class="interfaceViewportControl">Add Attachment</td>' +
						'</tr>';
				
		aHTML[++h] = '<tr><td>&nbsp;</td></tr>';

		aHTML[++h] = '<tr id="trInterfaceViewportControl" class="interfaceViewportControl">' +
						'<td id="tdInterfaceViewportControlDocuments" class="interfaceViewportControl">Pages</td>' +
						'</tr>';
		...
		//Like event attendees - need to do the same with attachments.				
	...
	aHTML[++h] = '<div id="divInterfaceMainDocuments" class="divInterfaceViewportMain"></div>';	
	...
	$('#tdInterfaceViewportControlDocuments').click(function(event)
	{
		interfaceMasterMainViewportShow("#divInterfaceMainDocuments");
		interfaceSetupWebsiteDocuments({actions: {add: true}});
	});
...
function interfaceSetupWebsiteDocuments(aParam, oResponse)
{
	var iObjectContext = giObjectContext;
	var sXHTMLElementId = 'divInterfaceMainDocuments';
	...
	if (oResponse == undefined)
	{
		...
		$.ajax(
		{
			...
			url: '/ondemand/setup/?' + sParam,
			dataType: 'json',
			success: function(data) {interfaceSetupWebsiteDocuments(aParam, data)}
		});
	...
	else
	{
		if (oActions != undefined)
		{
			...
			.click(function() {
				 interfaceMasterPagesAdd(aParam);
			})
		...
		if (oResponse.data.rows.length == 0)
	{
		...
		else
		{
			...
			{	
				...
				aHTML[++h] = '<td id="tdWebsitePages_Options-' + this.document + 
									'-' + this.id +
			...
			if (oOptions.view) 
			{
				...
				.click(function() {
					...
					interfaceDocumentSearch(this.id)
				})
				.css('width', '15px')
				.css('height', '17px')
			}	
			...
			if (oResponse.status == 'OK')
			{
				...
				$.ajax(
				{
					...
					success: function(oResponse) {
												...
												goObjectContext.headerscripts = (oResponse.data.rows[0]).headerscripts;
											}
				});
