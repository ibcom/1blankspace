...
ns1blankspace.product = 
{
	...
	show:	function (oParam, oResponse)
	{
		...
		else
		{
			...
			ns1blankspace.product.pricing.history(oParam)
		}
	},
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			if (ns1blankspace.objectContextData.currentretailprice != '')
			{
				...
				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Sell Price</td></tr>' +
	...
	{			
		_show: function ()
		{
			if ($('#ns1blankspaceMainPricing').attr('data-loading') == '1')
			{
				...
				aHTML.push('<table class="ns1blankspaceContainer">' +
								'<tr class="ns1blankspaceContainer">' +
								'<td id="ns1blankspacePricingColumn1" class="ns1blankspaceColumn1" style="width:160px;"></td>' +
								'<td id="ns1blankspacePricingColumn2" class="ns1blankspaceColumn2" style="width:160px;"></td>' +
								'<td id="ns1blankspacePricingColumn3" class="ns1blankspaceColumn2"></td>' +
								'</tr>' +
								'</table>');
				...
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								...
								'</td></tr>' +
								'<tr class="ns1blankspace">' +
								'<td class="ns1blankspaceText" id="ns1blankspacePricingTaxType">' +
								'</td></tr>');	
				...
				var iTaxType = (ns1blankspace.objectContextData && ns1blankspace.objectContextData.customerordertaxtype) 
								? ns1blankspace.objectContextData.customerordertaxtype : '1';

				ns1blankspace.financial.util.tax.codes(
				{
					xhtmlElementID: 'ns1blankspacePricingTaxType',
					id: iTaxType
				});
				
				ns1blankspace.product.pricing.group({xhtmlElemendID: 'ns1blankspacePricingColumn2'})
				ns1blankspace.product.pricing.history();

				if (ns1blankspace.objectContextData != undefined)
				{
					$('#ns1blankspacePricingPriceRetail').val(ns1blankspace.objectContextData.currentretailprice);
					$('#ns1blankspacePricingTaxType').val(ns1blankspace.objectContextData.currentretailprice);
				}
			}	
		},

		history: function(oParam)
		{
			var aHTML = [];
			var oSearch = new AdvancedSearch();
			oSearch.method = 'PRODUCT_PRICE_SEARCH';
			oSearch.addField('effectivefromdate,effectivetodate,price,costprice');
			oSearch.addFilter('product', 'EQUAL_TO', ns1blankspace.objectContext);
			oSearch.rows = 100;
			oSearch.sort('effectivefromdate', 'desc');
			oSearch.getResults(function(oResponse)
			{
				$('#ns1blankspacePricingColumn3').html('');
				...
				if (oResponse.status == 'OK')
				{
					if (oResponse.data.rows.length > 0)
					{
						ns1blankspace.objectContextData.currentretailprice = _.first(oResponse.data.rows).price;

						ns1blankspace.util.onComplete(oParam)

						if (false && oParam.onComplete != undefined)
						{
							ns1blankspace.util.onComplete(oParam)
						}
						else
						{
							aHTML.push('<table class="ns1blankspaceColumn2">');
							aHTML.push('<tr><td colspan="4" class="ns1blankspaceHeaderCaption" style="border-bottom: 0px;">' +
											'<strong>Pricing History</strong></td></tr>');
							aHTML.push('<tr>' +
											'<td class="ns1blankspaceHeaderCaption">Effective From</td>' + 
											'<td class="ns1blankspaceHeaderCaption">Effective To</td>' + 
											'<td class="ns1blankspaceHeaderCaption" style="text-align:right;">Sell Price</td>' + 
											'<td class="ns1blankspaceHeaderCaption" style="text-align:right;">Cost Price</td>' + 
										'</tr>');

							$.each(oResponse.data.rows, function()
							{
								aHTML.push('<tr>' + 
												'<td class="ns1blankspaceRow">' + ns1blankspace.util.fd(this.effectivefromdate, {format: 'DD MMM YYYY h:mm A'}) + '</td>' +
												'<td class="ns1blankspaceRow">' + ns1blankspace.util.fd(this.effectivetodate, {format: 'DD MMM YYYY h:mm A'}) + '</td>' +
												'<td class="ns1blankspaceRow" style="text-align:right;">' + this.price + '</td>' +
												'<td class="ns1blankspaceRow" style="text-align:right;">' + this.costprice + '</td>' +
											'</tr>');
							});

							aHTML.push('</table>');

							$('#ns1blankspacePricingColumn3').html(aHTML.join(''));
						}
					}
					else
					{
	...
	{			
		...
		process:	function (oResponse)
		{
			if (oResponse.status == 'OK')
			{
				...
				if ($('#ns1blankspaceMainPricing').html() != '')
				{
					ns1blankspace.product.save.priceHistoryCheck();
				}
