...
ns1blankspace.advancedSearch.util =
{
	objects: function (sDescription, oResponse)
	{
		if (oResponse == undefined)
		{
			var oSearch = new AdvancedSearch();
			oSearch.method = 'CORE_OBJECT_SEARCH';
			oSearch.addField('bulkavailable,description,id,prefix,snapshotavailable');
			if (sDescription != undefined)
			{
				oSearch.addFilter('description', 'TEXT_IS_LIKE', sDescription);
			}
				
			oSearch.sort('description', 'asc');
			oSearch.rows = 999;
			oSearch.getResults(function(data) {ns1blankspace.advancedSearch.util.objects(sDescription, data)});
		}
		else
		{
			if (oReponse.status == 'OK')
			{
				console.log(oResponse.data.rows)
			}
			else
			{
