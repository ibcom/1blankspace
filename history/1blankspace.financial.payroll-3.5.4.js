										...
										else
										{
											...
											if (ns1blankspace.financial.payroll.data.employeePayFrequencies.length > 0)
											{
												...
												$.each(oResponse.data.rows, function (r, row)
												{
													if (r == 0)
													{
														$vq.add(row.count + ' employees on ' + row.payfrequencytext.toLowerCase()
															, {queue: 'dashboard'});
													}
									...
									_init: function (oParam, oResponse)
									{
										...
										else
										{
											...
											else
											{
												...
												var aActions = $.grep(ns1blankspace.financial.payroll.data.payPeriodID, function (action) {return action.objectcontext == iPayPeriod})
					...
					check: 	function (oParam, oResponse)
								{
									if (oResponse == undefined)
									{
										...
										oSearch.addFilter('email', 'EQUAL_TO', ns1blankspace.user.email);
