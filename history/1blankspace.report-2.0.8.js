...
ns1blankspace.report = 
{
	...
	initData: function (oParam)
	{
		...
		[
			...
			*/
			...
			/* Actions */
			{
				...
				selectableParameters:
				{
					fields:
					[
						{name: "action.contactbusinesstext", columns: "tradename"},
						{name: "action.contactpersontext", columns: "firstname-space-surname"},
			...
			/* Attachments 322*/
			{
				id: 120,
				name: "Attachments",
				summary: "Detailed list of attachments.",
				group: 3,
				object: 322,
			...
			*/
			...
			/* Credit Notes */
			{
				...
				returnParameters: 'creditnote,creditnote.lineitem',	
		...
		[ 
			...
			{name: "action.contactbusinesstext", caption: "Business", columns: "tradename"},
			...
			$.each(ns1blankspace.report.reports, function() {
				...
				$($.grep(ns1blankspace.extend.structure, function (a) {return a.object == iObject;})).each(function(i,v)
				{
					$(v.elements).each(function(j,k)
					{
						...
						if (k.datatype == 2) {	
							ns1blankspace.report.dictionary.push({name: sObjectname + '.se' + k.reference + 'text', caption: sCaption});
						}	
						else {
							ns1blankspace.report.dictionary.push({name: sObjectname + '.se' + k.reference, caption: sCaption});
						}
					});
				})	
			});
	...
	home: function (oParam, oResponse)
	{
		...
		if ($.grep(ns1blankspace.report.reports, function(x) {return x.group != undefined}).length == ns1blankspace.report.reports.length)
		{
			...
			{	
				...
				aGroups = $.map(aGroups, function(x) {delete(x.tableAdded); return x});

				// Limit list of groups to those used in reports and then construct accordion
				var aUsedGroups = $.grep(ns1blankspace.report.reports, function(x) {return x.group});
				aGroups = $.grep(aGroups, function(x) {return $.inArray(x.id, $.map(aUsedGroups, function(x) {return x.group})) > -1});
		...
		// Now contsruct the report detail selection area
		var aHTML = [];
	...
	show:		
	{
		main: function (oParam, oResponse)
		{
			...
			if (ns1blankspace.report.config.autoRun != undefined && ns1blankspace.report.config.autoRun)
			{
				ns1blankspace.report.show.outputDivs();
				aHTML.push(ns1blankspace.report.show.configurationHeader({searchAtTop: bSearchAtTop}) + '</table>');
					...
					if (ns1blankspace.objectExtended)
					{
						...
						$.each(ns1blankspace.report.config.returnParameters.split(','), function()
						{
							...
							$($.grep(ns1blankspace.extend.structure, function (a) {return a['structureobjectlink.object.prefix'].toLowerCase() == sThisTable})).each(function(i,v)
							{
								$(v.elements).each(function(j,k)
								{
									sName = sThisPrefix + '.se' + k.reference;
			...
			$.each(ns1blankspace.report.config.allParameters, function()
			{
				...
				if (this.source != 1 && !bSelectableHeaderShown)
				{
					aHTML.push('<tr><td style="color:#B8B8B8; padding:4px; background-color:#F8F8F8; vertical-align:middle;">' +
									...
									((false) ? '<span id="spanReportSearch" class="ns1blankspaceAction">Search</span>' : '') +
									'</td></tr>');
				...
				{
					sName = this.name;
					oColumnOptions = $.grep(oResponse.data.parameters, function(x) {return x.name === sName}).shift();
					...
					{
						aHTML.push('<tr><td colspan="5" class="ns1blankspaceRow ns1blankspaceReportParameterFixed">' +
										sCaption + '</td>' +
									'</tr>');
					}
		...
		configurationHeader: function(oParam)
		{
			...
			{
				aHTML.push('<tr><td colspan="3"' +
								' style="color:#545454; padding:4px; background-color:#EAEAEA; vertical-align:middle;border-bottom:2px solid #E8E8E8; border-collapse:collapse;">' +
								'</td>');
			}
			else if (ns1blankspace.report.config.summary)
			{
	...
	search: 	
	{
		...
		show: function(oParam, oResponse)
		{
			...
			if (oResponse != undefined)
			{
				...
				else
				{
					...
					if (ns1blankspace.report.config.scriptOpen)
					{
		...
		row:		function (oRow)
		{
			// v2.0.3b Needed to add 1 to page seelcted instead of remove
			var aHTML = [];
			...
			if (bContinue)
			{
				...
				$.each(oOutputParameters, function()
				{
					...
					if (this.hidden === undefined || !this.hidden)
					{
						...
						sScriptNewPage = (ns1blankspace.report.config.scriptOnNewPage) ? ns1blankspace.report.config.scriptOnNewPage.toString() : undefined;
	...
	"export":	function (oParam)
	{
		...
		if (oParam)
		{
			if (oParam.exportStep === undefined) {oParam.exportStep = 1;}
		...
		// Call CORE_SEARCH_MORE in groups of 50 rows until we reach the total number of rows
		else if (oParam.exportStep === 2)
		{
			// Check to see if we already obtained all the data via a scriptReprotSearch / Process
		...
		else if (oParam.exportStep === 3)
		{
			...
			aHTML.push('<td class="ns1blankspaceTextMulti">' +
							'<div id="ns1blankspaceFileContents" class="ns1blankspaceTextMulti"' + 
	...
	update:		
	{
		...
		send: 		function (oParam)
		{
			...
			{						
				var sTotal = oResponse.summary.count;
