...
function interfaceSetupUserHomeShow()
{
...
function interfaceSetupUserSearch(sXHTMLElementId, iSource, sSearchText, sSearchContext)
{
	...
	if (sSearchContext != undefined && iSource != giSearchSource_BROWSE)
	{
		...
		var sParam = 'method=SETUP_USER_SEARCH&advanced=1&select=' + giSetupContext;
...
function interfaceSetupUserSearchShow(oXML)
{
	...
	else
	{
		...
		for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
		{
			...
			aHTML[++h] = '<td class="interfaceSearch" id="' + onDemandXMLGetData(oRow, "xhtmlcontext") +
							...
							onDemandXMLGetData(oRow, "username") + '</td>';
...
function interfaceSetupUserShow(oXML)
{
	...
	else
	{
		...
		var sContext = onDemandXMLGetData(oRow, 'username')
...
function interfaceSetupUserSummary()
{
	...
	else
	{
		...
		if (onDemandXMLGetData(oRow, 'disabled') == 'Y')
		{
		...
		aHTML[++h] = '<tr><td id="tdInterfaceMainSummarySiteID" class="interfaceMainSummary">User</td></tr>' +
						...
						onDemandXMLGetData(oRow, 'username') +
						'</td></tr>';
		...
		aHTML[++h] = '<tr><td id="tdInterfaceMainSummaryLastLogon" class="interfaceMainSummary">Name</td></tr>' +
						...
						onDemandXMLGetData(oRow, 'contactpersontext') +
						'</td></tr>';
		...
		if (onDemandXMLGetData(oRow, 'browsertext') != '')
		{
			aHTML[++h] = '<tr><td id="tdInterfaceMainSummaryLastBrowser" class="interfaceMainSummary">Last Accessed Using Browser</td></tr>' +
						...
						onDemandXMLGetData(oRow, 'browsertext') +
						'</td></tr>';
		}				
		...
		if (1==0 && onDemandXMLGetData(oRow, 'disabled') == 'N')
		{
...
function interfaceSetupUserDetails()
{
	...
	if ($('#divInterfaceMainDetails').html()  == gsLoadingXHTML)
	{
		...
		aHTML[++h] = '<tr id="trInterfaceMainDetailsUsername" class="interfaceMain">' +
						...
						'<input onDemandType="TEXT" id="inputInterfaceMainDetailsUserName" class="inputInterfaceMainText">' +
						'</td></tr>';
		...
		aHTML[++h] = '<tr id="trInterfaceMainDetailsDisabledText" class="interfaceMain">' +
						...
						'<textarea rows="10" cols="35" onDemandType="TEXTMULTI" id="inputInterfaceMainDetailsDisabledText" class="inputInterfaceMainTextMulti"></textarea>' +
						'</td></tr>';
		...
		if (oRoot.childNodes.length != 0)
		{
			$('#inputInterfaceMainDetailsUserName').val(onDemandXMLGetData(oRow, 'username'));
		}
