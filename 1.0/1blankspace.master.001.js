...
function interfaceMasterViewportShow(asReturn)	
{
	...
	$('#tdInterfaceMasterHeaderColumn2').html('<span id="spanInterfaceMasterViewportLogonName">' + aValues[3] + '</span>')
	
	if (aValues[0] == 'OK')
	{
		//aHTML[++h] = '<div id="divInterfaceMasterViewportSpaceName" class="interfaceMasterViewportLogo"></div>';
		//aHTML[++h] = '<div id="divInterfaceMasterViewportLogonName" class="interfaceMasterViewport">' + aValues[3] + '</div>';
	}
	else
	{
		//aHTML[++h] = '<div id="divInterfaceMasterViewportSpaceName" class="interfaceMasterViewport">[Space Name]</div>';
		//aHTML[++h] = '<div id="divInterfaceMasterViewportLogonName" class="interfaceMasterViewport">[User Name]</div>';
	}
	
	//aHTML[++h] = '<div id="divInterfaceMasterViewportSite" class="interfaceMasterViewport">&nbsp;</div>';
...
function interfaceMasterAttachmentsShowRow(oRow)
{
	...
	aHTML[++h] = '<td id="tdAttachment_filename-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
						'<a href="' + onDemandXMLGetData(oRow, "link") + '">' + onDemandXMLGetData(oRow, "filename") + '</a></td>';
						
	aHTML[++h] = '<td id="tdAttachment_date-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' + onDemandXMLGetData(oRow, "modifieddate") + '</td>';
	
	aHTML[++h] = '<td id="tdAttachment_options_delete-' + onDemandXMLGetData(oRow, "attachment") + 
...
function interfaceMasterAttachmentsUpload(aParam)
{
	...
	aHTML[++h] = '<form name="frmonDemandFileUpload" action="/directory/ondemand/attach.asp" ' +
					...
					'<input type="hidden" name="linktype" id="linktype" value="' + iObject + '">' +
					'<input type="hidden" name="linktypetext" id="linktypetext" value="' + sObjectName + '">' +
					'<input type="hidden" name="linkid" id="linkid" value="' + lObjectContext + '">';
...
function interfaceMasterElementEditStart(sElementId)
{
	...
	sHTML = '<input onDemandType="TEXT" id="' + sElementInputId + '" class="inputInterfaceMainValue" ' +
							'value="' + sHTML + '">'
	...
	else
	{
		...
		for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
		{
			...
			aHTML[++h] = '<td class="interfaceSearch" id="' + sElementId +
									...
									'-' + onDemandXMLGetData(oRow, "contactbusiness") +
			...
			if ($('#divInterfaceMasterSearch-' + iStartRow).length == 0)
			{
				...
				for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
				{
					...
					for (var i = 0; i < aColumns.length; i++)
					{
						...
						aHTML[++h] = '<td class="' + sClass + '" id="' + onDemandXMLGetData(oRow, "xhtmlcontext") +
											sXHTMLElementId + '-' + onDemandXMLGetData(oRow, "id") + sIDData + sIdAdditional + '">' 
...
{	
	...
	else
	{
		...
		for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
		{
			...
			aHTML[++h] = '<tr class="interfaceAttachments">';
			aHTML[++h] = '<td id="tdAction_subject-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
							onDemandXMLGetData(oRow, "subject") + '</td>';
			aHTML[++h] = '<td id="tdAction_date-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
							onDemandXMLGetData(oRow, "actiondate") + '</td>';
			
			if (bDescription)
			{
				aHTML[++h] = '<td id="tdAction_description-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
								onDemandXMLGetData(oRow, "description") + '</td>';
			}					
			
			aHTML[++h] = '<td id="tdAction_options-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRowOptionsSelect">&nbsp;</td>';
			aHTML[++h] = '</tr>'
		}
