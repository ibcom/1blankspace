...
function interfaceProjectTaskHomeShow(oXML)
{
...
function interfaceProjectTaskSearchShow(aParam, oXML)
{
...
function interfaceProjectTaskShow(aParam, oXML)
{
...
function interfaceProjectTaskSummary()
{
	...
	else
	{
		...
		if (onDemandXMLGetData(oRow, 'percentagecomplete') != '')
		{
			aHTML[++h] =  '<tr><td id="tdInterfaceMainSummaryPercentageComplete" class="interfaceMainSummary">Percentage Complete</td></tr>' +
						...
						onDemandXMLGetData(oRow, 'percentagecomplete') +
						'</td>' +
						'</tr>';
		}
...
function interfaceProjectTaskDetails()
{
	...
	if ($('#divInterfaceMainDetails').attr('onDemandLoading') == '1')
	{
		...
		aHTML[++h] = '<tr id="trInterfaceMainDetailsPercentageComplete" class="interfaceMain">' +
						...
						'<input onDemandType="TEXT" id="inputInterfaceMainDetailsPercentageComplete" class="inputInterfaceMainText">' +
						'</td></tr>';									
		...
		if (oRoot.childNodes.length != 0)
		{
			...
			$('#inputInterfaceMainDetailsTitle').val(onDemandXMLGetData(oRow, 'surname'));
			$('[name="radioStatus"][value="' + onDemandXMLGetData(oRow, 'status') + '"]').attr('checked', true);
			$('#inputInterfaceMainDetailsPercentageComplete').val(onDemandXMLGetData(oRow, 'percentagecomplete'));
...
function interfaceProjectTaskNew(aParam, oXML)
{
	if (oXML == undefined)
	{
		...
		$.ajax(
		{
			...
			dataType: 'xml',
			success: function(data) {interfaceProjectTaskNew(aParam, data)}
		});
	}	
	else	
	{
