...
function interfaceSetupNetworkGroupHomeShow(oXML)
{
	...
	if (oXML == undefined)
	{
		...
		var sParam = 'method=SETUP_NETWORK_GROUP_SEARCH';
		...
		$.ajax(
		{
			...
			dataType: 'xml',
			success: interfaceSetupNetworkGroupHomeShow
		});
	...
	else
	{
		...
		else
		{
			...
			for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
			{
				...
				aHTML[++h] = '<td id="interfaceNetworkGroupHomeMostLikely_Title-' + onDemandXMLGetData(oRow, "id") + 
...
function interfaceSetupNetworkGroupSearch(sXHTMLElementId, iSource, sSearchText, sSearchContext)
{
	...
	if (sSearchContext != undefined && iSource != giSearchSource_BROWSE)
	{
		...
		var sParam = 'method=SETUP_NETWORK_GROUP_SEARCH&select=' + giObjectContext;
		...
		$.ajax(
		{
			...
			url: '/directory/ondemand/setup.asp?rf=XML&' + sParam,
			dataType: 'xml',
			success: interfaceSetupNetworkGroupShow
		});
	}
	else
	{
...
function interfaceSetupNetworkGroupSearchShow(oXML)
{
...
function interfaceSetupNetworkGroupShow(oXML)
{
...
function interfaceSetupNetworkGroupSummary()
{
	...
	else
	{
		...
		if (onDemandXMLGetData(oRow, 'contactsynchronisation') == 'Y')
		{
...
function interfaceSetupNetworkGroupDetails()
{
	...
	if ($('#divInterfaceMainDetails').html()  == gsLoadingXHTML)
	{
		...
		if (oRoot.childNodes.length != 0)
		{
			...
			$('[name="radioContactSync"][value="' + onDemandXMLGetData(oRow, 'contactsynchronisation') + '"]').attr('checked', true);
		}
...
function interfaceSetupNetworkGroupSave()
{
	...
	{
		type: 'POST',
		url: '/directory/ondemand/setup.asp?' + sParam,
		data: sData,
		dataType: 'text',
...
function interfaceSetupNetworkGroupMembersAdd(oXML)
{
	...
	if (oXML == undefined)
	{
		...
		aHTML[++h] = '<tr id="trInterfaceMainMembersAddContactBusiness" class="interfaceMain">' +
						...
						'<input onDemandType="SELECT" id="inputInterfaceMainMembersAddContactBusiness" class="inputInterfaceMainSelect"' +
							' onDemandMethod="/directory/ondemand/object.asp?rf=XML&method=BUSINESS_SEARCH' +
		...
		aHTML[++h] = '<tr id="trInterfaceMainMembersAddSyncUser" class="interfaceMain">' +
						...
						'<input onDemandType="TEXT" id="inputInterfaceMainMembersAddSyncUser" class="inputInterfaceMainText">' +
						'</td></tr>';
		...
		.click(function() 
		{
			var sParam = 'method=PERSON_SEARCH&rf=XML&rows=2&quicksearch=Auto';
	...
	else
	{
		...
		else
		{
			...
			for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
			{
				...
				sData += '&usercontactperson=' +  onDemandXMLGetData(oRow, "id");
...
function interfaceSetupNetworkGroupNew(oXML)
{
	if (oXML == undefined)
	{
		...
		$.ajax(
		{
			...
			dataType: 'xml',
			success: interfaceSetupNetworkGroupNew
		});
	}	
	else	
	{
		...
		interfaceSetupNetworkGroupViewport();
		interfaceMasterMainViewportShow("#divInterfaceMainDetails", true);
		$('#spanInterfaceMasterViewportControlAction').button({disabled: false});
		$('#spanInterfaceMasterViewportControlActionOptions').button({disabled: true});
		interfaceSetupNetworkGroupDetails();
	}	
