...
function interfaceSetupProjectTaskHomeShow(oXML)
{
	...
	else
	{
		...
		else
		{
			...
			for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
			{
				...
				aHTML[++h] = '<td id="interfaceSetupProjectTaskHomeMostLikely_Title-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceHomeMostLikely">' +
...
function interfaceSetupProjectTaskSearch(sXHTMLElementId, aParam)
{
	...
	else
	{
		...
		if (sSearchText.length >= iMinimumLength || iSource == giSearchSource_BROWSE)
		{
			...
			var sParam = 'method=PROJECT_TASK_SEARCH&rf=XML&quicksearch' + sQuickSearchType + '=' + sSearchText + 
								'&xhtmlcontext=' + sXHTMLElementId;

			$.ajax(
			{
				type: 'GET',
				url: '/ondemand/SetupProjectTask/?' + sParam,
				dataType: 'xml',
...
function interfaceSetupProjectTaskSearchShow(aParam, oXML)
{
...
function interfaceSetupProjectTaskShow(aParam, oXML)
{
...
function interfaceSetupProjectTaskSummary()
{
	...
	else
	{
		...
		{
			interfaceSetupProjectMasterViewport();
			interfaceSetupProjectSearch('-' + onDemandXMLGetData(oRow, 'project'))
		});
	}	
}
...
function interfaceSetupProjectTaskDetails()
{
	...
	if ($('#divInterfaceMainDetails').attr('onDemandLoading') == '1')
	{
		...
		aHTML[++h] = '<tr id="trInterfaceMainProjectTaskDetailsTaskDependsOn" class="interfaceMain">' +
							...
							'<input onDemandType="SELECT" id="inputInterfaceMainProjectTaskDetailsTaskDependsOn" class="inputInterfaceMainSelect"' +
		...
		if (oRoot.childNodes.length != 0)
		{
			...
			$('#inputInterfaceMainProjectTaskDetailsType').attr('onDemandID', onDemandXMLGetData(oRow, 'type'));
			$('#inputInterfaceMainProjectTaskDetailsType').val(onDemandXMLGetData(oRow, 'typetext'));
			$('#inputInterfaceMainProjectTaskDetailsTaskBy').attr('onDemandID', onDemandXMLGetData(oRow, 'taskbyuser'));
			$('#inputInterfaceMainProjectTaskDetailsTaskBy').val(onDemandXMLGetData(oRow, 'taskbyusertext'));
			$('#inputInterfaceMainProjectTaskDetailsTaskDependsOn').attr('onDemandID', onDemandXMLGetData(oRow, 'templatedependsontask'));
			$('#inputInterfaceMainProjectTaskDetailsTaskDependsOn').val(onDemandXMLGetData(oRow, 'templatedependsontasktext'));
			
			$('[name="radioStartBasedOn"][value="' + onDemandXMLGetData(oRow, 'templateelapsedstartdatetype') + '"]').attr('checked', true);
		
			$('#inputInterfaceMainProjectTaskDetailsStartDays').val(onDemandXMLGetData(oRow, 'templateelapsedstartdatedurationdays'));
			$('#inputInterfaceMainProjectTaskDetailsDurationDays').val(onDemandXMLGetData(oRow, 'templateelapseddurationdays'));
			$('#inputInterfaceMainProjectTaskDetailsDisplayOrder').val(onDemandXMLGetData(oRow, 'displayorder'));
		}
	}	
}
...
function interfaceSetupProjectTaskActionDetails(aParam, oXML)
{
	...
	if (oXML == undefined && lAction != undefined)
	{
	...
	else
	{
		...
		aHTML[++h] = '<tr id="trInterfaceMainProjectTaskDetailsSubject" class="interfaceMain">' +
							...
							'<input onDemandType="TEXT" id="inputInterfaceMainProjectTaskDetailsSubject" class="inputInterfaceMainText">' +
							'</td></tr>';		
		...
		aHTML[++h] = '<tr id="trInterfaceMainProjectTaskDetailsDescriptionValue" class="interfaceMainTextMulti">' +
							...
							'<textarea rows="10" cols="50" style="width:100%;" onDemandType="TEXTMULTI" ' +
		...
		aHTML[++h] = '<tr id="trInterfaceMainProjectTaskDetailsType" class="interfaceMain">' +
							...
							'<tr id="trInterfaceMainProjectTaskDetailsTypeValue" class="interfaceMainSelect">' +
							'<td id="tdInterfaceMainProjectTaskDetailsTypeValue" class="interfaceMainSelect">' +
							'<input onDemandType="SELECT" id="inputInterfaceMainProjectTaskActionDetailsType" class="inputInterfaceMainSelect"' +
		...
		aHTML[++h] = '<tr id="trInterfaceMainProjectTaskDetailsTaskBy" class="interfaceMain">' +
							...
							'<tr id="trInterfaceMainProjectTaskDetailsTaskByValue" class="interfaceMainSelect">' +
							'<td id="tdInterfaceMainProjectTaskDetailsTaskByValue" class="interfaceMainSelect">' +
							'<input onDemandType="SELECT" id="inputInterfaceMainProjectTaskActionDetailsTaskBy" class="inputInterfaceMainSelect"' +
		...
		if (oXML != undefined)
		{
			...
			{
				var oRow = oRoot.childNodes.item(0);
				$('#inputInterfaceMainProjectTaskDetailsSubject').val(onDemandXMLGetData(oRow, 'subject'));
				
				$('#inputInterfaceMainProjectTaskDetailsDescription' + giEditorCounter).val(interfaceMasterFormatXHTML(onDemandXMLGetData(oRow, 'description')));	
				
				$('#inputInterfaceMainProjectTaskActionDetailsType').attr('onDemandID', onDemandXMLGetData(oRow, 'type'));
				$('#inputInterfaceMainProjectTaskActionDetailsType').val(onDemandXMLGetData(oRow, 'typetext'));
				$('#inputInterfaceMainProjectTaskActionDetailsTaskBy').attr('onDemandID', onDemandXMLGetData(oRow, 'actionby'));
				$('#inputInterfaceMainProjectTaskActionDetailsTaskBy').val(onDemandXMLGetData(oRow, 'actionbyfirstname') + ' ' + onDemandXMLGetData(oRow, 'actionbysurname'));
			}
		}	
...
function interfaceSetupProjectTaskActionsShow(oXML, sXHTMLElementID)
{	
	...
	else
	{
		...
		oRows.each(function() 
		{
			...
			aHTML[++h] = '<td style="width: 120px;" id="tdAction_subject-' + $(this).find('id').text() + '" class="interfaceMainRow">' +
							$(this).find('subject').text() + '</td>';
			
			aHTML[++h] = '<td id="tdAction_description-' + $(this).find('id').text() + '" class="interfaceMainRow">' +
								interfaceMasterFormatXHTML($(this).find('description').text()) + '</td>';
			
			aHTML[++h] = '<td class="interfaceMainRow" style="width: 40px;text-align:right;"><span id="tdSetupProjectTaskActions_delete-' + $(this).find('id').text() + '" class="interfaceMainRowOptionsDelete">&nbsp;</span>';
			aHTML[++h] = '<span id="tdSetupProjectTaskActions_select-' + $(this).find('id').text() + '" class="interfaceMainRowOptionsSelect">&nbsp;</span></td>';
				
			aHTML[++h] = '</tr>'
		});

//			aHTML[++h] = '<td style="width: 50px;" id="tdSetupProjectTaskActions_delete-' + $(this).find('id').text() + '" class="interfaceMainRowOptionsDelete">&nbsp;</td>';
//			aHTML[++h] = '<td style="width: 25px;" id="tdSetupProjectTaskActions_select-' + $(this).find('id').text() + '" class="interfaceMainRowOptionsSelect">&nbsp;</td>';
