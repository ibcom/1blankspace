...
function interfaceMasterWebsitePagesAdd(aParam, oResponse)
{
	...
	if (oResponse == undefined)
	{
		...
		aHTML[++h] = '<tr id="trInterfaceMainSetupWebsitePageAddDetails">' +
			...
			aHTML[++h] = '<td id="tdInterfaceMainSetupWebsitePageAddTypeValue" class="interfaceMainRadio">' +
							'<input type="radio" id="radioType5" name="radioType" value="5"/>HTML' +
							'<br /><input type="radio" id="radioType6" name="radioType" value="6"/>Text' +
							'</td>';
		...
		.click(function() 
		{
			...
			sData += '&documenttype=' + interfaceMasterFormatSave($('input[name="radioType"]:checked').val());
			sData += '&documentpublic=' + interfaceMasterFormatSave($('input[name="radioPublic"]:checked').val());
			
			if (parseInt($('input[name="radioType"]:checked').val()) == 5)
			{
	...
	else
	{
		if (oResponse.data.rows.length != 0)
		{
			...
			$('[name="radioType"][value="' + oObjectContext.documenttype + '"]').attr('checked', true);
