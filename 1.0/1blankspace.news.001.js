...
function interfaceNewsHomeShow(oXML)
{
	...
	if (oXML == undefined)
	{
		...
		{
			type: 'GET',
			url: '/ondemand/news/?rf=XML&' + sParam,
			dataType: 'xml',
...
function interfaceNewsSearch(sXHTMLElementId, aParam)
{
	...
	else
	{
		...
		if (sSearchText.length >= iMinimumLength || iSource == giSearchSource_BROWSE)
		{
			...
			var sParam = 'method=NEWS_SEARCH&rf=XML&advanced=1&quicksearch=' + sSearchText + 
...
function interfaceNewsSearchShow(aParam, oXML)
{
...
function interfaceNewsShow(aParam, oXML)
{
	...
	else
	{
		...
		aHTML[++h] = '<tr><td id="tdInterfaceMainSummarySummary" class="interfaceMainSummary">&nbsp;</td></tr>' +
						'<tr><td id="tdInterfaceMainSummarySummaryValue" class="interfaceMainSummaryValue">' +
						 interfaceMasterFormatXHTML(onDemandXMLGetData(oRow, 'news')) +
						'</td></tr>';
		...
		aHTML[++h] = '<tr><td id="tdInterfaceMainSummarySendAsEmail" class="interfaceMainColumn2Action">' +
						'<a href="#" id="aInterfaceMainSummarySendAsEmail">Send&nbsp;as&nbsp;email</a>' +
						'</td></tr>';
...
function interfaceNewsDetails()
{
	...
	if ($('#divInterfaceMainDetails').attr('onDemandLoading') == '1')
	{
		...
		if (oRoot.childNodes.length != 0)
		{
			...
			$('[name="radioTracking"][value="' + onDemandXMLGetData(oRow, 'tracking') + '"]').attr('checked', true);
		}
		else
		{
...
function interfaceNewsEdit()
{
	...
	if ($('#divInterfaceMainEdit').attr('onDemandLoading') == '1')
	{
		...
		{
			var oRow = oRoot.childNodes.item(0);
			$('#inputInterfaceMainEditText').val(unescape( interfaceMasterFormatXHTML(onDemandXMLGetData(oRow, 'news'))));
		}
...
function interfaceNewsNew(aParam, oXML)
{
	if (oXML == undefined)
	{
		...
		$.ajax(
		{
			...
			dataType: 'xml',
			success: function(data) {interfaceNewsNew(aParam, data)}
		});
	}	
	else	
	{
...
function interfaceNewsCopy(aParam, oXML)
{
	if (giObjectContext != -1)
	{
		if (oXML == undefined)
		{
			var sParam = 'method=NEWS_COPY&rf=XML';
		...
		{
			giObjectContext = $(oXML).find('id').text()
			interfaceNewsShow(aParam, oXML)
			...
			$('#divInterfaceMainDetails').attr('onDemandLoading', '1');
			interfaceNewsDetails();
			if (bCopy) {interfaceMasterStatus('Copied');} */
...
function interfaceNewsGroupsContacts(aParam, oXML)
{
	...
	if (oXML == undefined)
	{
		var sParam = 'method=NEWS_CONTACT_PERSON_SEARCH&rows=30' +
						'&id=' + giObjectContext;
		
		$.ajax(
		{
			type: 'GET',
			url: '/ondemand/news/?rf=XML',
			data: sParam,
			dataType: 'xml',
	...
	else
	{
		...
		{		
			...
			{
				var oRow = oRoot.childNodes.item(iRow);
				aHTML[++h] = interfaceNewsGroupsContactsRow(oRow);
			}
...
function interfaceNewsGroupsContactsRow(oRow)
{
	...
	aHTML[++h] = '<td id="tdNewsGroupsList_contact-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
							onDemandXMLGetData(oRow, "firstname") + '</td>';
							
	aHTML[++h] = '<td id="tdNewsGroupsList_activity-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
							onDemandXMLGetData(oRow, "surname") + '</td>';
							
	aHTML[++h] = '<td id="tdNewsGroupsList_link-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
...
function interfaceNewsGroupsManage(aParam, oXML)
{
	...
	if (oXML == undefined)
	{
		var sParam = 'method=NEWS_PERSON_GROUP_SEARCH&rows=20' +
						'&news=' + giObjectContext;
		
		$.ajax(
		{
			type: 'GET',
			url: '/directory/ondemand/news.asp?rf=XML',
			data: sParam,
			dataType: 'xml',
	...
	else
	{
		...
		aHTML[++h] = '<table id="tableInterfaceMainNewsGroups" class="interfaceMain">' +
					'<tr id="trInterfaceMainNewsGroupsRow1" class="interfaceMainRow1">' +
					'<td id="tdInterfaceMainNewsGroupsColumn1" class="interfaceMainColumn1Large">' +
					'</td>' +
					'<td id="tdInterfaceMainNewsGroupsColumn2" style="width: 100px;" class="interfaceMainColumn2Action">' +
					'</td>' +
					'</tr>' +
					'</table>';				
		...
		//aHTML[++h] = '<tr><td id="tdInterfaceMainNewsGroupsAdd" class="interfaceMainAction">' +
		//				'<span id="spanInterfaceMainNewsGroupsAdd">Add Group</span>' +
		//				'</td></tr>';
			...
			for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
			{
				...
				else
				{
					...
					if (onDemandXMLGetData(oRow, "grouptext") != '')
					{
						...
						aHTML[++h] = '<td id="tdNewsGroupsList-title-' + onDemandXMLGetData(oRow, "group") + '" class="interfaceMainRow">' +
												onDemandXMLGetData(oRow, "grouptext") + '</td>';
												
												
						aHTML[++h] = '<td id="tdNewsGroupsList-' + onDemandXMLGetData(oRow, "group") + '" class="interfaceMainRowOptionsSelect">&nbsp;</td>';
...
function interfaceNewsGroupsAdd(aParam, oXML)
{
...
function interfaceNewsGroupsAddSelect(sXHTMLElementId)
{
	...
	$.ajax(
		{
			type: 'POST',
			url: '/directory/ondemand/news.asp?' + sParam,
			data: sData,
			dataType: 'text',
...
function interfaceNewsTrackingSummary(aParam, oXML)
{
	...
	else
	{
		...
		else
		{
			...
			for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
			{
				...
				aHTML[++h] = '<td id="tdNewsGroupsList_activity-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
										onDemandXMLGetData(oRow, "activity") + '</td>';
										
				aHTML[++h] = '<td id="tdNewsGroupsList_link-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
										onDemandXMLGetData(oRow, "link") + '</td>';
				
				aHTML[++h] = '<td id="tdNewsGroupsList_link-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
										onDemandXMLGetData(oRow, "count") + '</td>';
...
function interfaceNewsTrackingDetails(aParam, oXML)
{
	...
	else
	{
		...
		{		
			...
			for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
			{
				...
				aHTML[++h] = '<td id="tdNewsGroupsList_link-' + onDemandXMLGetData(oRow, "id") + '" class="interfaceMainRow">' +
										onDemandXMLGetData(oRow, "datetime") + '</td>';
...
function interfaceNewsSave()
{
	...
	{
		sData += '&news=' + encodeURIComponent(tinyMCE.get('inputInterfaceMainEditText').getContent());
	}
