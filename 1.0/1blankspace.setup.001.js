...
function interfaceSetupMasterViewport(aParam)
{
	...
	$.ajax(
	{
		...
		url: sParam,
		dataType: 'xml',
		success: function(data) {interfaceSetupHomeShow(aParam, data)}
	});

}

function interfaceSetupHomeShow(aParam, oXML)
{
	...
	else
	{
		...
		for (var iRow = 0; iRow < oRoot.childNodes.length; iRow++) 
		{
			...
			aHTML[++h] = '<td id="tdSetup-' + onDemandXMLGetData(oRow, "id") + 
							'" class="interfaceMainRow interfaceHomeMostLikely">' +
							onDemandXMLGetData(oRow, "title") + '</td>';
			
			aHTML[++h] = '<td style="width:23px;text-align:right;" id="tdSetup_delete-' + onDemandXMLGetData(oRow, "id") + 
...
function interfaceSetupRemove(sXHTMLElementId)
{
	...
	{
		var sParam = gsSetupMethod + '_MANAGE&remove=1';
...
function interfaceSetupElementEditSave(sElementId)
{
	...
	var sParam = gsSetupMethod + '_MANAGE&id=' + aElement[1];
