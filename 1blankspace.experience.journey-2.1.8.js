...
ns1blankspace.experience.journey =
{
	...
	init: 		function (oParam) 
				{
					...
					[
						...
						{
							what: 'click',
							where: 'td.ns1blankspaceExperienceExplore',
							subject: 'explore'
						},
						{
							what: 'click',
							where: 'div.ns1blankspaceExperiencePopulateSwitch',
							subject: 'populateSwitch'
						},
						{
							what: 'click',
							where: 'td.ns1blankspaceExperienceSwitch',
							subject: 'switch'
						}					
					]);

					ns1blankspace.experience.journey.prepare.affects();
					ns1blankspace.experience.journey.prepare.routes();
	...
	get: 		function (oParam)
				{
					...
					if (sRouteID === undefined && ns1blankspace.util.getParam(oParam, 'xhtmlElementID').exists)
					{
						sRouteID = $('#' + ns1blankspace.util.getParam(oParam, 'xhtmlElementID').value).attr('data-route-id');
					}
						
					if ($.grep(ns1blankspace.experience.journey.data.routes, function(a) {return a.routeID == sRouteID}).length !==0)
					{
						oJourney = {routeID: sRouteID};
	...
	set: 		function (oParam)
				{
					var oJourney = ns1blankspace.experience.journey.get({routeID: oParam.routeID});
					var oDestination = ns1blankspace.util.getParam(oParam, 'destination').value;
					var oPreviousDestination = ns1blankspace.util.getParam(oParam, 'previousDestination').value;

					if (oJourney !== undefined)
					{
						if (oDestination !== undefined)
						{
							oJourney.destination = $.extend(true, oJourney.destination, oDestination)
						}

						if (oPreviousDestination !== undefined)
						{
							oJourney.previousDestination = $.extend(true, oJourney.previousDestination, oPreviousDestination)
						}	
					}	
				},			
					...
					routes: 	function (sNamespace, sRouteID)
				  				{
				  					...
				  					{
				  						sNamespace = 'ns1blankspace.data.control.experience.origins';
									...
									for (var m in oNamespace)
									{
										...
										{	
											...
											ns1blankspace.experience.journey.data.routes.push(
											{
												routeID: oNamespace[m]['routeID'],
												level: oNamespace[m]['level']
											});
										}	
									}
				  	...
				  	affects: function (sNamespace, bPreserve)
				  				{
				  					...
				  					if (!bPreserve) {ns1blankspace.experience.journey.data.affects = []};
									...
									for (var m in oNamespace)
									{
										...
										{	
											...
											{
												ns1blankspace.experience.journey.data.affects.push({routeID: v, effectedRoute: oNamespace});
											});
										}	
	...
	controller: {
					...
					message:	function (oParam)
								{
									...
									ns1blankspace.experience.journey.controller.data.messages.unshift($.extend(true, {}, oParam));
					...
					explore: 	function (oParam)
								{
									ns1blankspace.experience.journey.thingsToSee.thingToSee.explore(oParam);
								},
					...
					populateSwitch: 	
								function (oParam)
								{
									...
									//oParam = ns1blankspace.util.setParam(oParam, 'tenancy', 'multi-simple');
									ns1blankspace.experience.journey.thingsToSee.populate(oParam);
								},
					...
					switch: 	function (oParam)
								{
									...
									oParam = ns1blankspace.util.setParam(oParam, 'populateSwitchID', oInitiator.attr('data-populate-with-id'));

									var oJourney = ns1blankspace.experience.journey.get(oParam);

									ns1blankspace.util.execute(oJourney.destination.onSwitch, oParam);
								},																																	
				},
	...
	affects: 	function (oParam)
				{
					var sRouteID = ns1blankspace.util.getParam(oParam, 'routeID').value;
					var aAffects = $.grep(ns1blankspace.experience.journey.data.affects, function (a) {return (a.routeID == sRouteID)});

					$.each(aAffects, function (i, affect)
					{
						oParam = ns1blankspace.util.setParam(oParam, 'effectedDestination', affect.effectedRoute);
						ns1blankspace.util.execute(affect.effectedRoute.deriveWith, oParam);
					});
				...
				{ 	
					show: 		function (oParam)
								{
									...
									$.each(oOrigin.destinations, function(i, v)
									{
										aHTML.push('<div id="ns1blankspaceExperience-' + sOriginID + '-' + v.id + '"' +
														' data-travel-to-route-id="' + sOriginID + '-' + v.id + '"' +
														' style="padding:10px; font-size:1.2em; cursor: pointer;" class="ns1blankspaceExperienceTravelTo">' +
					...
					previous:
								function (oParam)
								{
									...
									$.each(oJourney.destinations.reverse(), function (i, destination)
									{
										if (oDestination == undefined && destination.populationAtRestLocation !== undefined)
										{
									...
									travelTo: 	function (oParam) 
												{
													...
													{	
														...
														if (oJourney.destination.tenancy == 'single')
														{	
	...
	thingsToSee:
				{
					populate: 	function (oParam, oResponse)
								{
									...
									var bGetData = ((oJourney.populateWith !== undefined || oJourney.destination.populationAtRestLocationURI !== undefined) && oResponse === undefined && !bExtend && (sTenancy == 'single' || sTenancy == 'multi' || sTenancy == 'switch'));
									...
									{	
										...
										else
										{
											...
											$.each($.grep(oJourney.destination.thingsToSee, function(a) {return a.model !== undefined}), function (i, v)
											{
												...
												if (v.thingsToSee && v.populationAtRestData == undefined)
												{
													$.each(v.thingsToSee, function (j, k)
													{
														if (k.model)  //&& k.populationAtRestLocation !== undefined
														{
															if (v.type == 'derived')
															{
									...
									else
									{
												...
												$.each(oJourney.destination.thingsToSee, function (i, thingToSee)
												{
													...
													{	
														...
														else
														{
															...
															aHTMLThing.push('<div style="clear:both;">');

															if (thingToSee.type == 'sign')
															{
																if (thingToSee.format == 'money')
																{	
																	sValue = ns1blankspace.util.formatMoney(sValue, {decimals: 2}).formatted
																}	

																aHTMLThing.push('<div class="ns1blankspaceCaption"' +
																				' id="ns1blankspaceExperience-' + oJourney.routeID + '-' + thingToSee.id + '"' +
																				' data-destination-id="' + thingToSee.id + '"' +
																				' data-route-id="' + oJourney.routeID + '-' + thingToSee.id + '"' +
										...
										{	
											...
											{	
												...
												$.each(oJourney.destination.thingsToSee, function (i, thingToSee)
												{
													aHTML.push('<td class="ns1blankspaceExperienceHeader"' +
																	' style="' + (thingToSee.style?ns1blankspace.experience.util.toNV(thingToSee.style):'') + '">' +
																	thingToSee.caption + '</td>');
												});
											...
											$.each(oJourney.destination.thingsToSee, function (i, thingToSee)
											{
												...
												if (thingToSee.type == 'derived')
												{	
													sClass = 'ns1blankspaceExperienceExplore'
												}
												...
												aHTML.push('<td class="' + sClass + '"' +
																...
																' data-destination-id="' + thingToSee.id + '"' +
																' style="height:30px; ' +
																(thingToSee.style?ns1blankspace.experience.util.toNV(thingToSee.style):'') + '">' +
																'</td>');
											});
										...
										else if (sTenancy == 'multi' && !bExtend)
										{
											...
											else
											{
												...
												aHTML.push('<div class="ns1blankspaceExperienceContainer"><table style="width:100%;">');
												...
												$.each(oJourney.destination.populationAtRest, function (i, v)
												{
													...
													$.each(oJourney.destination.thingsToSee, function (j, thingToSee)
													{
														...
														if (thingToSee.model)
														{	
															if (thingToSee.thingsToSee && thingToSee.type != 'derived' && thingToSee.populationAtRestLocation !== undefined)
															{	
																var aValues = [];
															...
															else if (thingToSee.thingsToSee && thingToSee.type != 'derived' && thingToSee.populationAtRestData !== undefined)
															{
																sValue = $.grep(thingToSee.populationAtRestData, function (a) {return a.id == v[thingToSee.model]})[0].title;
																iPopulateWithID = v[thingToSee.model];
															}
															else
															{
										...
										else if (sTenancy == 'switch')
										{
											...
											$.each(oJourney.destination.populationAtRest, function (i, populationData)
											{
												iColumn = iColumn + 1;

												sClass = '';

												aHTML.push('<td class="ns1blankspaceExperienceSwitch' + sClass + '" id="ns1blankspaceExperience-' + oJourney.routeID + '-' + populationData.id + '" ' +
																	' style="font-size:0.875em; text-align:left; width:' + (100/oJourney.destination.populationAtRest.length) + '%"' +	
																	' data-populate-with-id="' + populationData.id + '" data-route-id="' + oJourney.routeID + '" data-destination-id="' + oJourney.destinationID + '">')

												//' data-populate-id="' + oParam.populateID + '" ' +

												$.each(oJourney.destination.thingsToSee, function (i, thingToSee)
												{
													aHTML.push(populationData[thingToSee.model]);
												});
					...
					refresh: 	function (oParam)
								{									
									...
									var oPreviousDestination = ns1blankspace.experience.journey.destinations.previous(oParam);
									var sRouteID = oPreviousDestination.routeID;
									var sPopulateID = (oParam.populateID != undefined?'-' + oParam.populateID:'');
									var sXHTMLElementID = 'ns1blankspaceExperience-' + sRouteID;
									var bSendMessage = ns1blankspace.util.getParam(oParam)

									for (var key in oPopulationBeingRested)
							  		{
							     		if (oPopulationBeingRested.hasOwnProperty(key))
							     		{
						     				var oJourney = ns1blankspace.experience.journey.get({routeID: sRouteID + '-' + key});
						     				...
						     				{	
						     					if (oJourney.destination.thingsToSee !== undefined && oJourney.destination.type != 'derived')
						     					{
													...
													$.each(oJourney.destination.thingsToSee, function (i, v)
													{
														if (v.model) {aValues.push(oPopulationBeingRested[key + '.' + v.model])}
													});

													sValue = aValues.join(' ');
						     					}
						     					else if (oJourney.destination.format === 'money')
							     				{	
							     					sValue = ns1blankspace.util.formatMoney(sValue, {currency: '', decimals: 2}).formatted;
							     				}
							     			}
							     			
						     				$('#' + sXHTMLElementID + '-' + key + sPopulateID).html(sValue);
							     		}
							     	}
					...
					thingToSee: {			
									...
									populate: 	function (oParam) 
												{
													...
													var sXHTMLPopulateElementID = 'ns1blankspaceExperience-'

													if (oParam.populateRouteID)
													{	
													 	sXHTMLPopulateElementID += oParam.populateRouteID; 
													}
													else
													{
														sXHTMLPopulateElementID += oJourney.previousRouteID; 
													}	

													if (oParam.populateID!==undefined) {sXHTMLPopulateElementID = sXHTMLPopulateElementID + '-' + oParam.populateID}

													oParam.populationAtWorkToBeRested = $.extend(true, oParam.populationAtWorkToBeRested, $.grep(oData, function(a) {return a.id == sPopulateWithID})[0]);
									...
									explore: 	function (oParam, oResponse) 
												{
													var oJourney = ns1blankspace.experience.journey.get(oParam);
													var sXHTMLElementID = ns1blankspace.util.getParam(oParam, 'xhtmlElementID').value;
													var sClass;
									...
									rest: 		function (oParam, oResponse) 
												{
													...
													{	
														...
														var iDestination = oJourney.destinations.length - 2;

														if (oJourney.previousDestination.type == 'derived')
														{
															iDestination = iDestination - 1;
														}
														
														var sPutToRest = oJourney.destinations[iDestination].populationAtRestLocation + '_MANAGE';
														...
														else
														{
															...
															if (oJourney.destination.type == 'select')
															{		
																oData[(oJourney.destination.model).split('.')[1] || oJourney.destination.model] = oPopulationAtWorkToBeRested.id;
															...
															{	
																...
																$.each(oJourney.previousDestination.populationAtWorkToBeRestedWhenCan, function (i, whenCan)
																{
																...
																{	
																	$.each(oJourney.previousDestination.populateWith, function (i, populateWith)
																	{
																		oData[(populateWith.model).split('.')[1] || populateWith.model] = populateWith.value;
																	});
																}
															}	
														}	
													...
													else
													{
														...
														else
														{
															...
															else
															{
																...
																{
																	if ($('#ns1blankspaceExperience-' + oJourney.previousRouteID + '-new-container').length != 0)
																	{
																		$('#ns1blankspaceExperience-' + oJourney.previousRouteID + '-new-container [data-populate-id=""]').each(function (i, v)
																		{
																			$(v).attr('data-populate-id', oResponse.id);
																			$(v).attr('id', $(v).attr('id') + oResponse.id);
																		});

																		oParam.populateID = oResponse.id;

																		$('#ns1blankspaceExperience-' + oJourney.previousRouteID + '-new-container').css('background-color', '');
																	}	
																	...
																	var aPopulationAtRest = $.grep(oPreviousDestination.populationAtRest, function(a) {return a.id == oPopulationBeingRested.id});

																	if (aPopulationAtRest.length == 0)
																	{	
																		oPreviousDestination.populationAtRest.push(oPopulationBeingRested);
																	}
																	else
																	{
