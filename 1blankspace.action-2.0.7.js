...
ns1blankspace.action = 
{
	...
	details: function ()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			{
				$('#ns1blankspaceDetailsDate').val($.fullCalendar.formatDate(new Date(), 'dd MMM yyyy h:mm TT'));
	...
	{	// v2.0.4 Added Reminders
		...
		{	// Show list of reminders linked to the action
			...
			if (!$('ns1blankspace.ns1blankspaceRemindersColumn1').is('*'))
			{
		...
		{	// Allow user to edit details of the reminder
			...
			else
			{
				...
				else
				{
					...
					$('#ns1blankspaceReminderEditSendAt').val(((sSendAtDate == undefined) ?  $.fullCalendar.formatDate(new Date(), 'dd MMM yyyy h:mm TT') : sSendAtDate));
		...
		{	
			...
			{ 
				$('#' + sXHTMLElementID).fullCalendar('addEventSource', 
				{
					...
					{
						//console.log(moment(start, ns1blankspace.option.dateFormats).format('YYYY-MM-DD HH:mm:ss'));
						//console.log(moment(end, ns1blankspace.option.dateFormats).format('YYYY-MM-DD HH:mm:ss'));
						...
						else
						{
							...
							$.ajax(
							{
						      url: ns1blankspace.util.endpointURI('ACTION_CALENDAR_SEARCH'),
						      dataType: 'json',
						      data: oData,
						      success: function(oResponse)
						      {
									var events = [];
									$.each(oResponse, function(r, row)
									{
										if (false)
										{
							          	events.push(
							         ...
							         {
							         	row.start = moment(row.start, ns1blankspace.option.dateFormats).format('YYYY-MM-DD HH:mm:ss');
							         	//row.end = moment(row.end, ns1blankspace.option.dateFormats).format('YYYY-MM-DD HH:mm:ss');
							         	events.push(row);
							         } 	
						        	});
									callback(events);
