		...
		attachments:
		{
			show: function(oParam, oResponse)
			{
				...
				if (oResponse == undefined)
				{
					...
					oSearch.addField('post.attachment.filename,post.attachment.download,post.attachment.createdusertext,' +
									 'post.attachment.createduser,post.attachment.createddate,post.attachment.attachment,post.attachment.id,post.attachment.sourcetype');
			...
			row: function(oRow, oParam)
			{
				...
				aHTML.push('<td id="ns1blankspaceAttachment_filename-' + oRow['post.attachment.id'] + '" class="ns1blankspaceRow" style="font-size: 0.75em;">' +
									'<a href="' + oRow['post.attachment.download'] + '" class="ns1blankspaceNoUnloadWarn">' + oRow['post.attachment.filename'] + '</a></td>');
				...
				else
				{
					...
					{	
						...
						var sHTML = '<table class="ns1blankspaceContainer">' +
									'<tr class="ns1blankspaceContainer">' +
									'<td id="ns1blankspaceCommentsColumn1-' + sKey + '" class="ns1blankspaceColumn1Flexible" style="font-size:0.725em; background-color:white;padding-left:5px;">'
										+ sMessage + '</td>' +
						...
						setTimeout(ns1blankspace.messaging.conversation.posts.comments.showMessage, 700, oParam)
					}
		...
		show: function (oParam)
		{
			...
			{	
				...
				oSearch.addField(sObjectPrefix + '.attachment.type,' + sObjectPrefix + '.attachment.filename,' + 
										...
										sObjectPrefix + '.attachment.sourcetype,' +
				...
				oSearch.getResults(function(oResponse) 
				{
					if (oResponse.status == 'OK')
					{
						oResponse.data.rows = $.map(oResponse.data.rows, function(row)
						{
							var y = {};
					...
					oResponse.data.rows = $.grep(oResponse.data.rows, function (oRow) {return (oRow.attachment != '')});
					...
					ns1blankspace.attachments.process(oResponse, oParam)
				});
			}
		}
	}	
