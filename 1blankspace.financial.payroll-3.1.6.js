	...
	summary: function (oParam, oResponse)
				{
					...
					else
					{
						...
						if (false && ns1blankspace.objectContextData.status == 2)
						{
							aHTML.push('<tr><td><span id="ns1blankspaceSummaryTotals"' +
													...
													' class="ns1blankspaceRowTotals"></span></td></td></tr>');
						}
						...
						$('#ns1blankspaceSummaryTotals').button(
						{
							label: 'Totals & ' + ns1blankspace.option.taxOffice + ' Reporting'
						})
						.click(function()
	...
	pays: 		function (oParam, oResponse)
				{
					...
					{	
						if (oResponse == undefined)
						{
							...
							oSearch.addField('grosssalary,calculations,netsalary,deductions,allowances,superannuation,calculations,taxbeforerebate,notes,' +
	...
	show: 		function (oParam, oResponse)
				{
					...
					if (oResponse == undefined)
					{
						...
						oSearch.addField('payrecord.employee.contactperson,payrecord.employee.contactpersontext,payrecord.employee.employeenumber,' +
											'sum(grosssalary) grosssalary,sum(netsalary) netsalary,sum(deductions) deductions,sum(allowances) allowances,sum(superannuation) superannuation,sum(taxbeforerebate) taxbeforerebate');
	...
	employees: 	
				{
									...
									else
									{
										...
										else
										{
											...
											ns1blankspace.render.page.show(
										   {
											...
											aHTML.push('<tr><td class="ns1blankspaceCaption" style="padding-top:14px;">' +
														ns1blankspace.option.taxOffice + ' Reporting</td></tr>');

											aHTML.push('<tr>' +
														'<td class="ns1blankspaceSubNote" style="padding-top:4px;">' +
													...
													if (iStep == 2)
													{
														...
														{	
															...
															else
															{
																...
																var oSearch = new AdvancedSearch();
																oSearch.method = 'FINANCIAL_PAYROLL_PAY_RECORD_SEARCH';
																oSearch.addField('grosssalary,netsalary,deductions,allowances,superannuation,taxbeforerebate,payrecord.payperiod.paydate');
																oSearch.addSummaryField('sum(grosssalary) grosssalary');
																oSearch.addSummaryField('sum(netsalary) netsalary');
																oSearch.addSummaryField('sum(superannuation) superannuation');
																oSearch.addSummaryField('sum(taxbeforerebate) taxbeforerebate');
																...
																oSearch.getResults(function(oResponse)
																{
																	...
																	{	
																		...
																		ns1blankspace.financial.payroll.data.summaries[iDataIndex].deductions = (oResponse.summary.deductions).parseCurrency().formatMoney(0, '.', ',');
					...
					report: 	{
												...
												$.each(oSummaries, function (s, oSummary)
												{
													...
													$.each(oItem.fields, function (f, oField)
													{
														...
														else
														{
															...
															{
																oItemData[oField.name] = oParam[oField.param];
															}
														...
														if (oField.onlyIfData && (oItemData[oField.name] == undefined || oItemData[oField.name] == 0)
														{
									...
									submit:
									{
										...
										send: function (oParam)
										{
											var azureLoginUrl = "https://login.microsoftonline.com/singletouch.onmicrosoft.com/oauth2/v2.0/authorize";
    										azureLoginUrl    += "?p=b2c_1_singletouch&client_id=SingleTouch-Unique-Client-Id-Here";
										}
									}
								}								
				}
