	...
	data: {
				...
				[
					...
					},
					{
						...
						objectParameterObjects: 'contactperson,persongroup'
					},
					{
						object: 277, 
					...
					init: 	function (oParam)
								{
									...
									else
									{
										...
										aHTML.push('<table><tr>' +
														'<td class="ns1blankspaceRow" style="width:15px;"><span id="ns1blankspaceFileImportObjectBack"></span></td>' +
	...
	"export":{
					...
					data: 	{
									get: function (oParam)
									{
										...
										{
											oRender = ns1blankspace.data.render[sXHTMLContext];
										}

										if (oRender != undefined)
										{
											iMore = oRender.more;
											if (iMore == undefined) {iMore = oRender.moreid}
												
											if (oRender.summary != undefined)
											{
												iRows = oRender.summary.count;
											}
										}
					...
					process: function (oParam)
								{
									...
									if (oFormat.length > 0)
									{
										...
										var _data = ns1blankspace.setup.file['export']._data;

										_data.totalRecords = oParam.totalRecords;
										_data.file = [];
										_data.start = 1
										...
										$.each(oFormat.header, function(i, k)
										{
											$.each(k.fields, function (j, v)
											{
												...
												if (oOptions.delimiter != undefined && j < k.fields.length - 1)
												{
												...
												_data.current =
												{
													text: ns1blankspace.util.format(v),
													length: v.length,
													start: _data.start
												}

												if (_data.current.length == undefined)
												{
										...
										if (oFormat.item !== undefined)
										{
											$.each(oItems, function (i, oItem)
											{
												$.each(oFormat.item[0].fields, function (j, v)
												{
													...
													if (oOptions.delimiter != undefined && j < oFormat.item[0].fields.length - 1)
													{
														aFile.push(oOptions.delimiter);
													}
					...
					saveToFile: function(oParam)
								{
									...
									if (sFileName == undefined) {sFileName = ns1blankspace.util.getParam(oParam, 'filename', {"default": 'export.csv'}).value;}
	...
	util: 	{
					getFields: 	function (oParam, oResponse)
								{	
									...
									returnParameters = ns1blankspace.setup.file["import"].data.objectName
									...
									else
									{
										...
										{
											var sPrefix = v['structureobjectlink.object.prefix'].toLowerCase();
											$(v.elements).each(function(j,k)
											{
												var p = {};
												var sAlias = ((k.alias != '') ? k.alias : 'se' + k.id).toLowerCase();

												p.name = sPrefix + '.' + sAlias;
												p.caption = sAlias + ' (' + k.title + ')';
												p.inputtype = 'textbox';
										...
										if (oParam.type == 'template')
										{
											 ns1blankspace.setup.file["import"].template.init(oParam);
										}

										if (oParam.type == 'upload')
										{
											 ns1blankspace.setup.file["import"].upload.show(oParam);
										}
									}	
								},
...
[
	{
		...
		specification: 'https://softwaredevelopers.ato.gov.au/PSARspecification',
