							...
							if (iStep == 1)
							{
								if (oResponse == undefined)
								{
									if (ns1blankspace.financial.data.bankaccounts == undefined || bRefresh)
									{
										...
										oSearch.addField('title,lastreconciledamount,lastreconcileddate,notes,status,financialaccount,accountname,bsb,accountnumber');
							...
							{	
								...
								else if (iType == 2)
								{
									...
									ns1blankspace.financial.unallocated.row = function (oRow)
									{
										...
										aHTML.push('<td style="width:30px;text-align:right;" class="ns1blankspaceRow">');
										aHTML.push('<span id="ns1blankspaceReconcileItems_options_search-' + oRow["receipt.lineitem.id"] + '-1"' +
														' class="ns1blankspaceUnallocatedEdit"></span></td>');
										aHTML.push('</tr>');
	...
	edit: 		function(oParam, oResponse)
				{
					...
					{	
						...
						{	
							...
							var oData = $.grep(ns1blankspace.financial.data.accounts, function (a)
							{ 
								return (a.type == (iType==1?2:1) && a.postable == 'Y' && a.id != ns1blankspace.financial.unallocatedAccount)
							});
